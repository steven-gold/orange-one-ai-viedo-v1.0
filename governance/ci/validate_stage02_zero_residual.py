...
def main():
    ...
    if "state: STAGE1_VALIDATION_COMPLETED_CI_PASS" not in state:
        return fail("EXECUTION_STATE_NOT_STAGE1_CI_PASS")
    if "stage2_started: false" not in state:
        return fail("EXECUTION_STATE_STAGE2_NOT_FALSE")
    if "stage2_current:" in state or "current_stage2:" in state:
        return fail("EXECUTION_STATE_CONTAINS_STAGE2_CURRENT_PROJECTION")

    active = (ROOT / "governance/test/ACTIVE_STATE.yaml").read_text(encoding="utf-8")
    for token in (
        "current_stage: STAGE-01-CLOSED",
        "result: NOT_EXECUTED",
        "prior_results_authoritative_for_next_run: false",
        "status: ACTIVE_STAGE1_CLOSED_STAGE2_CLEARED",
    ):
