...
type Account={account_id:string;label:string;status:string;identity_source:string;organization_scope:string;mfa:string;risk:string;session:string;front_l1:string[];admin_l1:string[];basic_data:Record<string,string>};
...
type State={accounts:Account[];search_query:string;draft:Draft|null;preview:Preview|null;configured:boolean;applied_add:Set<string>;applied_remove:Set<string>;completed:boolean;mode:"CREATE"|"EDIT"|null;target_account_id:string|null;audits:string[]};
const initialAccounts:Account[]=[
 {account_id:"TEST-IAM-ACCOUNT-001",label:"[TEST] Existing operator A",status:"ACTIVE",identity_source:"TEST_IDENTITY_SOURCE",organization_scope:"TEST_SCOPE_A",mfa:"ENFORCED",risk:"LOW",session:"1 ACTIVE TEST SESSION",front_l1:["FRONT-L1-01","FRONT-L1-03"],admin_l1:["ADMIN-L1-IAM"],basic_data:{"TEST-IAM-FIELD-IDENTITY-CANDIDATE":"TEST-IDENTITY-CANDIDATE-001","TEST-IAM-FIELD-ORG-SCOPE":"TEST-ORG-SCOPE-A"}},
 {account_id:"TEST-IAM-ACCOUNT-002",label:"[TEST] Existing operator B",status:"ACTIVE",identity_source:"TEST_IDENTITY_SOURCE",organization_scope:"TEST_SCOPE_B",mfa:"ENFORCED",risk:"LOW",session:"0 ACTIVE TEST SESSION",front_l1:["FRONT-L1-06"],admin_l1:[],basic_data:{"TEST-IAM-FIELD-IDENTITY-CANDIDATE":"TEST-IDENTITY-CANDIDATE-002","TEST-IAM-FIELD-ORG-SCOPE":"TEST-ORG-SCOPE-B"}},
];
const state:State={accounts:initialAccounts.map(x=>({...x,front_l1:[...x.front_l1],admin_l1:[...x.admin_l1],basic_data:{...x.basic_data}})),search_query:"",draft:null,preview:null,configured:false,applied_add:new Set(),applied_remove:new Set(),completed:false,mode:null,target_account_id:null,audits:[]};
...
function modeState(create:string,edit:string){return state.mode==="EDIT"?edit:create;}
function projectionState(){if(state.completed)return"COMPLETE";if(state.preview)return modeState("CREATE_PREVIEW","EDIT_PREVIEW");if(state.draft?.validated)return modeState("CREATE_PERMISSION","EDIT_PERMISSION");if(state.draft)return modeState("CREATE_BASIC","EDIT_BASIC");return"LIST";}
...
export async function executeControlledIamOperation(r:IamRuntimeRequest):Promise<{ok:true;value:unknown;correlation_id:string}|{ok:false;reason_code:string;correlation_id:string}|null>{
 ...
 if(r.operation==="saveDraft"){
  ...
  state.mode=p.mode==="EDIT"?"EDIT":"CREATE";state.target_account_id=text(p.account_id)??(state.mode==="CREATE"?NEW_ACCOUNT_ID:null);
  state.draft={draft_id:text(p.draft_id)??"TEST-IAM-DRAFT-001",basic_data:Object.fromEntries(Object.entries(basic).filter(([,v])=>typeof v==="string")) as Record<string,string>,front_l1:strings(p.front_l1),admin_l1:strings(p.admin_l1),validated:false};state.preview=null;state.completed=false;audit(`draft saved ${state.draft.draft_id}`);return ok({draft_id:state.draft.draft_id,state:projectionState()},r.correlation_id);
 }
 if(r.operation==="validateDraft"){
  if(!state.draft||r.draft_id!==state.draft.draft_id)return fail("IAM-01-ERR-UNDEFINED",r.correlation_id);state.draft.validated=true;audit(`draft validated ${state.draft.draft_id}`);return ok({draft_id:state.draft.draft_id,state:projectionState()},r.correlation_id);
 }
 if(r.operation==="previewAuthorizationImpact"){
  ...
  const accountId=r.account_id??state.target_account_id??NEW_ACCOUNT_ID,desired=assignmentSet(front,admin),current=currentAssignmentSet(accountId),added=[...desired].filter(x=>!current.has(x)),removed=[...current].filter(x=>!desired.has(x)),unchanged=[...desired].filter(x=>current.has(x));
  state.target_account_id=accountId;state.draft.front_l1=front;state.draft.admin_l1=admin;state.preview={preview_ref:"TEST-IAM-PREVIEW-001",account_id:accountId,added,removed,unchanged,blocked:[],approval_required:true};state.configured=false;state.applied_add.clear();state.applied_remove.clear();audit(`authorization preview ${state.preview.preview_ref}`);return ok(state.preview,r.correlation_id);
 }
