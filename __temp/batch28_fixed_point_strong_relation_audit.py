...
BASE_HEAD="14effbe24e0ee80068e076ef5e4d0a3ca3f630af"
SYSTEM_DOCS=[
...
for field,page,uid,r in targets:
    ...
    for sr in sys_rows[owner]:
        v=sr.get(field,"")
        if sr.get("operation")==op and sr.get("runtime_status")==status and not missing(v) and v not in SENTINELS:
            candidates.append({"value":v,"control":sr["control"],"table":sr["table"],"row":sr["row"],
              "gate":sr.get("gate",""),"permission":sr.get("permission",""),"runtime_owner":sr.get("runtime_owner","")})
    vals=collections.defaultdict(list)
    for c in candidates: vals[c["value"]].append(c)
    if len(vals)==1:
        cls="STRONG_UNIQUE_RELATION";candidate=next(iter(vals))
    elif len(vals)>1:
        cls="STRONG_RELATION_CONFLICT";candidate=None
    else:
        cls="NO_STRONG_RELATION";candidate=None
    rows.append({"field":field,"page":page,"uid":uid,"operation":op,"runtime_status":status,
      "runtime_owner":r.get("runtime_owner",""),"canonical_owner":owner,"classification":cls,
      "candidate":candidate,"candidate_values":dict(vals)})

summary={
"target_cells":324,
"strong_unique_relation":sum(1 for x in rows if x["classification"]=="STRONG_UNIQUE_RELATION"),
"strong_relation_conflict":sum(1 for x in rows if x["classification"]=="STRONG_RELATION_CONFLICT"),
"no_strong_relation":sum(1 for x in rows if x["classification"]=="NO_STRONG_RELATION"),
...
"by_owner_class":{o:dict(collections.Counter(x["classification"] for x in rows if x["canonical_owner"]==o)) for o in sorted(set(x["canonical_owner"] for x in rows))},
"unique_candidate_controls":len(set((x["page"],x["uid"]) for x in rows if x["classification"]=="STRONG_UNIQUE_RELATION")),
}
assert summary["strong_unique_relation"]+summary["strong_relation_conflict"]+summary["no_strong_relation"]==324
report={"marker":"ACPOS-20260922-BATCH-28-FIXED-POINT-STRONG-RELATION-AUDIT-V1","base_head":BASE_HEAD,"summary":summary,"rows":rows}
Path("__batch28_strong_relation_audit.json").write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
Path("__batch28_summary.txt").write_text(json.dumps(summary,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
print("BATCH28_SUMMARY="+json.dumps(summary,ensure_ascii=False,sort_keys=True))
