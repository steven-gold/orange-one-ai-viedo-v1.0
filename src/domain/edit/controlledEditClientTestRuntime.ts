...
function internalAction(input:EditActionInvokeInput){
  ...
  if(input.action_uid==="EDIT-01-ACT-CORR-GENERATE"){correctionCandidateRef=ref("EDIT-CORRECTION-CANDIDATE");correctionApprovedRef=null;next.values={...next.values,"EDIT-01-LBL-API-JOB":correctionCandidateRef};return ok(next,input.action_uid,{candidate_ref:correctionCandidateRef});}
  if(input.action_uid==="EDIT-01-ACT-CORR-APPROVE"){if(!correctionCandidateRef)return fail("CORRECTION_CANDIDATE_REQUIRED","EDIT-01-ERR-CORRECTION-001");correctionApprovedRef=ref("EDIT-CORRECTION-APPROVED");return ok(next,input.action_uid,{approved_candidate_ref:correctionApprovedRef});}
  if(input.action_uid==="EDIT-01-ACT-CORR-EXECUTE"||input.action_uid==="EDIT-01-ACT-API-APPLY"){if(input.action_uid==="EDIT-01-ACT-CORR-EXECUTE"&&!correctionApprovedRef)return fail("APPROVED_CORRECTION_REQUIRED","EDIT-01-ERR-CORRECTION-001");next.page_state_uid="EDIT-01-ST-PAGE-EVAL_REQUIRED";next.current_stage_phase="EVALUATING";setGate(next,"EDIT-01-GATE-EVALUATION",true);setGate(next,"EDIT-01-GATE-STAGE-PASS",false);return ok(next,input.action_uid,{working_draft_mutated:true});}
