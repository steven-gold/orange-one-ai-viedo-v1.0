...
Each governed Page or System-Logic Unit owns an independent Stage lifecycle. Work Unit closure and that governed Unit's Stage/capability closure are different decisions, but Stage closure MUST be evaluated only against the Current governed Unit's applicable Stage denominator. A partial Work Unit MAY close when its exact included scope satisfies its Definition of Done, but it MUST persist remaining REQUIRED scope of that same governed Unit and MUST NOT grant that Unit Stage-exit credit while its own required Stage items remain. Other Pages, other System-Logic Units, unrelated captured sources, or project-wide page counts MUST_NOT be inserted into the Unit's Stage-exit denominator merely because they use the same Stage UID. Stage closure MUST reconcile the Current governed Unit's Stage-required universe against completed, formally non-applicable, blocked, and remaining items from Current scope/denominator Authority; it MUST_NOT infer completion from one successful subset of that Unit.
