...
check('HP-004', state.get('status') == 'ACTIVE_CORE01_STAGE03_VISUAL_REVIEW_PENDING' and state.get('next_action') == 'HUMAN_VISUAL_REVIEW_CORE01_STAGE03', 'CURRENT_STATE', f"status={state.get('status')} next={state.get('next_action')}")
...
check('HP-005', scope.get('closure_status') == 'STAGE03_VISUAL_REVIEW_PENDING' and scope.get('next_action') == 'HUMAN_VISUAL_REVIEW_CORE01_STAGE03' and (scope.get('remaining_units') == ['CORE-01']), 'PROJECTOR_CONTINUITY', f"scope closure={scope.get('closure_status')} next={scope.get('next_action')} remaining={scope.get('remaining_units')}")
...
check('HP-042', not visual_unresolved and set(map(str, docs.get('VISUAL_INHERITANCE_MATRIX', {}).get('unresolved_applicable_visual_authority_refs') or [])) == set() and (set((str(x.get('authority_ref')) for x in docs.get('VISUAL_INHERITANCE_MATRIX', {}).get('rows') or [] if isinstance(x, dict) and x.get('resolution') == 'RESOLVED_CURRENT_PHYSICAL_AUTHORITY')) == {'GLOBAL_HOME_SHELL_TEMPLATE_AUTHORITY@V1.9', 'GLOBAL_WEB_VISUAL_SYSTEM_AUTHORITY@V1.0'}) and (docs.get('VISUAL_INHERITANCE_MATRIX', {}).get('status') == 'PASS_READY_FOR_HUMAN_VISUAL_REVIEW') and (stage3.get('stage_exit_allowed') is False), 'VISUAL_AUTHORITY', f"unresolved_applicable_visual_authorities={[x.get('authority_ref') for x in visual_unresolved]}")
...
check('HP-043', findings_doc.get('open_gap_total') == 0 and findings_doc.get('closure_blocker_total') == 0 and (findings_doc.get('remaining_scope_total') == 1) and ((findings_doc.get('problems') or []) == []) and (findings_doc.get('human_visual_review_status') == 'PENDING') and (attempt.get('open_gap_total') == 0) and (attempt.get('closure_blocker_total') == 0) and (attempt.get('remaining_scope_total') == 1), 'DENOMINATOR_INTEGRITY', f'current_problem_uids={sorted(problem_refs)} visual_unresolved={len(visual_unresolved)}')
...
check('HP-051', review.get('review_result') == 'PENDING_HUMAN_VISUAL_REVIEW' and review.get('visual_approval') is False and (review.get('design_freeze_allowed') is False) and (set(map(str, review.get('resolved_authority_refs') or [])) == {'GLOBAL_HOME_SHELL_TEMPLATE_AUTHORITY@V1.9', 'GLOBAL_WEB_VISUAL_SYSTEM_AUTHORITY@V1.0'}), 'HUMAN_REVIEW_GATE', f"review={review.get('review_result')} approval={review.get('visual_approval')} freeze={review.get('design_freeze_allowed')}")
