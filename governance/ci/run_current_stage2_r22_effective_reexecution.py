...
RESULT = ROOT / ".github/stage02-test/STAGE02_R22_EFFECTIVE_REEXECUTION_RESULT.json"
...
def load_product():
    ...
    if len(out) != 38 or by_page != Counter({"ASSET-01": 33, "CORE-01": 5}):
        die(f"PRODUCT_DENOMINATOR_DRIFT:{len(out)}:{dict(by_page)}")
...
def effective(page, raw_scan, product, gap006):
    ...
    for gap in raw_scan.get("gaps") or []:
        ...
        if key in product:
            rem = product[key]
            prod_elim.append({"raw_gap": gap, "remediation_uid": rem.get("remediation_uid"), "source_cycle": rem.get("source_cycle", "R3_BOUNDED_FUNCTIONAL_COMPLETION"), "materialized_closure": rem.get("materialized_closure"), "effective_reproduction": False})
        elif key in gap006:
            auth_elim.append({"raw_gap": gap, "authority_gap_uid": "GAP-006", "resolution": "RESOLVED_EXACT_AUTHORITY", "effective_reproduction": False})
        else:
...
if os.environ.get("STAGE02_FULL_LINE_CONFIRMED") != "1":
    die("R22_REEXECUTION_REQUIRES_FULL_LINE_CONFIRMATION")
state = load(STATE)
freeze = load(FREEZE)
execution = state.get("execution") or {}
if execution.get("current_stage") != "STAGE-02-TESTED-BLOCKED" or (execution.get("stage2") or {}).get("result") != "TEST_EXECUTED_BLOCKED":
    die("R22_REEXECUTION_REQUIRES_STAGE02_BLOCKED_STATE")
if freeze.get("stage_uid") != "STAGE-02" or not freeze.get("frozen_governance_uid"):
    die("FROZEN_GOVERNANCE_RECEIPT_INVALID")
for validator in (
    ROOT / "governance/ci/validate_stage02_post_action_signal_roles_r22.py",
    ROOT / "governance/ci/validate_stage02_r22_materialized_closures.py",
...
product, product_by_page, cycle_counts = load_product()
gap006 = load_gap006()
...
for page, cfg in PAGES.items():
    ...
    pages[page] = {
        ...
        "functional_chain_effective_r22_scan": eff,
...
if product_eliminated != 38 or gap006_eliminated != 4 or effective_total != 129:
    die(f"R22_EFFECTIVE_ARITHMETIC:raw={raw_total}:product={product_eliminated}:gap006={gap006_eliminated}:effective={effective_total}")
if effective_total != raw_total - product_eliminated - gap006_eliminated:
    die("R22_EFFECTIVE_ARITHMETIC_IDENTITY_FAILED")
if union_gap_uids != EXPECTED_GAPS:
    die(f"EXTERNAL_AUTHORITY_UNION_DRIFT:{sorted(union_gap_uids)}")
all_categories = Counter()
for page in pages.values():
    all_categories.update(page["functional_chain_effective_r22_scan"]["gap_categories"])
if dict(sorted(all_categories.items())) != EXPECTED_EFFECTIVE_CATEGORIES:
    die(f"R22_EFFECTIVE_CATEGORY_DRIFT:{dict(sorted(all_categories.items()))}")
if any(g.get("category") == "SHARED_OWNER_AUTHORITY_UNRESOLVED" for p in pages.values() for g in p["functional_chain_effective_r22_scan"]["gaps"]):
...
result = {
    ...
    "reexecution_cycle": "R22_POST_ACTION_SIGNAL_ROLE_CORRECTION",
    ...
    "notes": [
        ...
        "Exactly 38 product materializations re-matched current fresh Raw signatures, including two R22 role-correct closures.",
        ...
        "Stage-02 remains blocked with 129 effective functional gaps; Stage-03, website construction, and deployment remain prohibited.",
    ],
}
...
print("PASS: validated product eliminations=38; GAP-006 eliminations=4")
print("PASS: fresh effective Stage-02 functional gaps=129")
print(f"PASS: effective categories={dict(sorted(all_categories.items()))}")
print("BLOCKED: one R22 post-action authority gap remains; Stage-02 / construction / deployment stay fail-closed")
