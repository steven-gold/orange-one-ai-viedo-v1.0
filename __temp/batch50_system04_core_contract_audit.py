...
BASE_HEAD="080e03e202859604c646fe55edc89c0f67737549"
...
S02=SYSTEM_DOCS[1];S03=SYSTEM_DOCS[2];S06=SYSTEM_DOCS[5];S08=SYSTEM_DOCS[7];S09=SYSTEM_DOCS[8]
PAIR_PRECEDENCE={
...
assert len(remaining)==190,len(remaining)
sys04=[x for x in remaining if x["canonical_owner"]==S04]
assert len(sys04)==53,len(sys04)
...
keywords=ops+[
"blueprint","canonical script","candidate","project draft","topic draft","story","story candidate","dna","lock",
"mother lock","child lock","review","human decision","approval","revision","evaluation","compare","project blueprint",
"script version","version","lineage","audit","evidence","state transition","persistence","immutable","source ref",
"project","topic","chapter","asset","visual","music","subtitle","voice","quality"
]
...
for ti,t in enumerate(d.tables):
    ...
    for ri,row in enumerate(t.rows[1:],2):
        ...
        if joined and rx.search(joined):
            evidence["rows"].append({"table":ti+1,"row":ri,"headers":h,"values":vals,"joined":joined[:8000]})

owner_rows=parse_controls(S04)
owner_exact={}
...
"operations":len(ops),
"by_page":dict(collections.Counter(x["page"] for x in sys04)),
"by_field":dict(collections.Counter(x["field"] for x in sys04)),
"by_runtime_status":dict(collections.Counter(x["runtime_status"] for x in sys04)),
"by_runtime_owner":dict(collections.Counter(x["runtime_owner"] for x in sys04)),
"by_operation":dict(collections.Counter(x["operation"] for x in sys04)),
"owner_paragraph_hits":len(evidence["paragraphs"]),
"owner_table_hits":len(evidence["rows"]),
}
report={"marker":"ACPOS-20260922-BATCH-50-SYSTEM04-CORE-CONTRACT-DESIGN-AUDIT-V1","base_head":BASE_HEAD,
"summary":summary,"operations":ops,"controls":list(controls.values()),"owner_exact_rows":owner_exact,
"owner_evidence":evidence,"system04_cells":sys04}
Path("__batch50_system04_core_contract_audit.json").write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
Path("__batch50_summary.txt").write_text(json.dumps({"summary":summary,"controls":list(controls.values())},ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
print("BATCH50_SUMMARY="+json.dumps(summary,ensure_ascii=False,sort_keys=True))
