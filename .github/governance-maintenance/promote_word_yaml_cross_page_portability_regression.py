...
OLD_UID='GOV-REV-20260923-WORD-YAML-PAGE-NEUTRAL-SCHEMA-PORTABILITY-HARDENING'
NEW_UID='GOV-REV-20260923-WORD-YAML-CROSS-PAGE-PORTABILITY-REGRESSION-CLOSURE'
OLD_DISPLAY='v2.2.19'
NEW_DISPLAY='v2.2.20'
OLD_REV='v2.2.19-word-yaml-cross-page-portability-regression-closure'
NEW_REV='v2.2.20-word-yaml-cross-page-portability-regression-closure'
AUTH_UID='USR-DIRECTIVE-20260923-WORD-YAML-CROSS-PAGE-PORTABILITY-REGRESSION-R3'
WORK_UNIT='WU-GOV-WORD-YAML-PRESTAGE01-HARDENING-001'
NEW_PACKAGE='AI_WEB_GOVERNANCE_FULL_LIFECYCLE_v2.2.20_WORD_YAML_CROSS_PAGE_PORTABILITY_REGRESSION_CLOSURE_LOCAL_VERIFIED.zip'
...
def harden_page_neutral_projection_policy():
    p=SOURCE/'12_DOCS/mother-spec/01_BLUEPRINT_DESIGN_GOVERNANCE.md'
    body=p.read_text(encoding='utf-8')
    anchor="The Canonical Source Projection MUST use one registered schema UID/revision and one fixed serialization contract. Top-level fields, nested object fields, row fields, sequence ordering, null representation, encoding, line ending, anchor/alias policy, and content-hash rules are part of the contract. A producer MUST NOT add ad-hoc fields, rename fields, silently omit fields, reorder governed sequences, or create a page-specific/product-specific alternative projection schema. A field that does not apply remains present with the contract-defined null/empty value where the schema requires fixed shape."
    add=anchor+"\n\nOnly the structural schema is fixed. Product/page content is dynamic and MUST be derived from each immutable source. Common policy MUST NOT hard-code page UID values, page-type enums, mandatory page-content values, Business Entity names, Function/Operation names, Control/Field names, visual-style values, source heading text, source paragraph/table/media counts, source-node counts, or any product-specific denominator. Required schema headings/keys MAY be fixed only as structural container identities; their product content and row multiplicity MUST remain source-derived. Different pages MAY contain materially different functions, tables, drawings, headings, sections, controls, and design concepts without changing the projection schema. A new page/content shape is represented by additional or different source rows inside the same registered structural envelopes, not by editing common policy."
    if 'Only the structural schema is fixed. Product/page content is dynamic' not in body:
        if anchor not in body: raise RuntimeError('Mother 01 S090 fixed-schema anchor missing')
        body=body.replace(anchor,add,1)
        p.write_text(body,encoding='utf-8')

    p=SOURCE/'12_DOCS/mother-spec/04_AUDIT_PROGRESS_STANDARD.md'
    body=p.read_text(encoding='utf-8')
    anchor="Negative regression MUST include at least: Word byte mutation after lock; missing projection row; duplicate row; row-order drift; extra/unregistered field; missing required field; relationship omission; package-part hash mismatch; projection hash drift; reconciliation self-declared PASS with nonzero mismatch; freeze receipt missing; pair-hash mismatch; projection mutation after freeze; direct selected source-intake/base-blueprint execution admission without a frozen pair; and semantic interpretation inserted into projection."
    add=anchor+" Portability regression MUST also prove that at least two materially different structured-document/page-content shapes use the same registered projection schema without common-policy edits, fixed page identities, fixed page-type enums, fixed product-function/control vocabularies, fixed source-heading values, or fixed source-content counts."
    if 'Portability regression MUST also prove that at least two materially different structured-document/page-content shapes' not in body:
        if anchor not in body: raise RuntimeError('Mother 04 S087 negative-regression anchor missing')
        body=body.replace(anchor,add,1)
        p.write_text(body,encoding='utf-8')

def ensure_cross_page_portability_regression():
    p=SOURCE/'09_TESTS/governance/test_stage1_source_to_blueprint_minimal_control.py'
    body=p.read_text(encoding='utf-8')
    if 'projection_schema_portable_across_materially_different_docx' in body:
        return
...
def mutate_policy():
    ...
    harden_page_neutral_projection_policy()
    ...
    inv['SOURCE_PROJECTION_FIDELITY']={
      ...
      'fixed_projection_scope':'SCHEMA_KEYS_TYPES_ORDER_AND_FIDELITY_INVARIANTS_ONLY',
      'page_product_content':'DYNAMIC_FROM_EACH_IMMUTABLE_RAW_SOURCE',
      'fixed_page_uid_or_page_type':'BLOCK',
      'fixed_product_function_control_or_source_content_count':'BLOCK',
      'cross_page_content_shape_portability_required':True
    }
    ...
    item15=next(x for x in aud.get('items',[]) if x.get('audit_item_uid')=='AUD-GOV-015')
    unique_extend(item15.setdefault('coverage_extensions',[]),['FIXED_SCHEMA_DYNAMIC_PAGE_CONTENT_PORTABILITY','NO_PRODUCT_PAGE_IDENTITY_OR_FIXED_CONTENT_DENOMINATOR'])
    ...
    bpd['structured_document_source_projection_audit_contract']={
      ...
      'fixed_schema_scope':'STRUCTURAL_KEYS_TYPES_ORDER_AND_FIDELITY_ONLY',
      'dynamic_page_content_required':True,
      'fixed_product_page_or_function_vocabulary':'BLOCK',
      'fixed_source_content_count':'BLOCK',
      'cross_page_materially_different_content_shape_portability_required':True
    }
    ...
    mutate_stage1_guard()
    mutate_stage1_tests()
    ensure_cross_page_portability_regression()

def mutate_projection_portability_validator():
    p=ROOT/'governance/ci/validate_governance_portability.py'
    body=p.read_text(encoding='utf-8')
    if 'source_projection_common_schema_product_neutrality' in body:
        return
    marker="out={\n"
    insert=r'''# Structured-document projection portability: fixed schema, dynamic page/product content.
...
'''
    if marker not in body: raise RuntimeError('portability validator output marker missing')
...
def mutate_current_components():
    ...
    d['pre_stage_source_projection_admission']={
      ...
      'source_intake_work_unit_activation_before_source_pair_frozen':'BLOCK',
      ...
      'source_intake_semantic_input':'EXACT_FROZEN_CANONICAL_SOURCE_PROJECTION_ONLY',
      'raw_word_semantic_reparse_during_source_intake':'BLOCK',
      'schema_alias_or_optional_field_fallback':'BLOCK',
      'not_applicable_requires_authority_evidence':True
    }
    wur=d.setdefault('work_unit_resolution_gate',{})
    for _old in ['stage01_source_projection_pre_activation_required_when_applicable','stage01_source_projection_freeze_receipt_required','stage01_source_projection_pair_hash_binding_required','stage01_unverified_projection_activation']:
        wur.pop(_old,None)
    wur['source_intake_projection_pre_activation_required_when_applicable']=True
    wur['source_intake_projection_freeze_receipt_required']=True
    wur['source_intake_projection_pair_hash_binding_required']=True
    wur['source_intake_unverified_projection_activation']='BLOCK'
    ...
    mutate_projection_portability_validator()
...
def prepare_source_candidate_state():
    ...
    g=load(gp)
    g['candidate']='v2.2.20_WORD_YAML_CROSS_PAGE_PORTABILITY_REGRESSION_CLOSURE_CANDIDATE'
...
def refresh_source():
    ...
    if '## v2.2.20 Word/YAML cross-page portability regression closure' not in rs:
    ...
    if '## v2.2.20 cross-page portability regression closure rule' not in vs:
...
def update_current(semantic,checks,bundle,zips):
    ...
    unique_extend(r['active_specification'].setdefault('aliases',[]),['word-yaml-cross-page-portability-regression-closure'])
    r['immediate_predecessor']={
      ...
      'status':'SUPERSEDED_HISTORY_ONLY_AFTER_WORD_YAML_CROSS_PAGE_PORTABILITY_REGRESSION_CLOSURE'
    }
    dump(rp,r)
    ...
    a['status']='WORD_YAML_CROSS_PAGE_PORTABILITY_REGRESSION_SUCCESSOR_PROMOTED_EXACT_HEAD_REVALIDATION_REQUIRED'
    a['next_action']='RUN_EXACT_HEAD_WORD_YAML_CROSS_PAGE_PORTABILITY_REGRESSION_VALIDATION'
    ...
    gt.update({
      ...
      'fresh_revalidation_scope':'WORD_YAML_CROSS_PAGE_PORTABILITY_REGRESSION_POLICY_CONSUMERS_ONLY_NO_PRODUCT_STAGE_CREDIT',
    ...
    rc['current_resume_point']='WORD_YAML_CROSS_PAGE_PORTABILITY_REGRESSION_SUCCESSOR_PROMOTED_EXACT_HEAD_REVALIDATION_REQUIRED'
    rc['current_work_unit_uid']=WORK_UNIT
    rc['exact_next_action']='RUN_EXACT_HEAD_WORD_YAML_CROSS_PAGE_PORTABILITY_REGRESSION_VALIDATION'
    ...
    sd['next_action']='RUN_EXACT_HEAD_WORD_YAML_CROSS_PAGE_PORTABILITY_REGRESSION_VALIDATION'
...
def main():
    ...
    validate_all()
    for p in [
      ROOT/'.github/workflows/word-yaml-cross-page-portability-regression-promotion.yml',
      ROOT/'.github/governance-maintenance/promote_word_yaml_cross_page_portability_regression.py'
    ]:
    ...
    msg='feat(governance): close Word YAML cross-page portability regression\n\nSpec-Change-Authorization: '+AUTH_UID+'\nSpec-Change-Scope: WORD_YAML_CROSS_PAGE_PORTABILITY_REGRESSION_CLOSURE'
