...
function ActionButton({ id, labelKey, primary = false, compact = false, disabled = false, onClick }: ControlProps) {
  ...
  return (
    <button
      ...
      data-runtime-binding={disabled ? "BUSY" : "ACTION_BOUND"}
...
export function CoreVisual() {
  ...
  const reportBlock = (reason: string) => {
    setPageState("ERROR");
  ...
  const runServerAction = async (
    ...
    options: { path_params?: Record<string, string>; query?: Record<string, string>; payload?: unknown } = {},
  ) => {
  ...
  const requireProject = (): string | null => {
    if (!clientState.project_ref) {
      reportBlock("CORE-01-ERR-CONTEXT-001:REQUIRED_PROJECT_REF_MISSING");
      return null;
    }
    return clientState.project_ref;
  };

  const requireTopic = (): string | null => {
    if (!clientState.topic_ref) {
      reportBlock("CORE-01-ERR-TOPIC-LINEAGE-001:REQUIRED_TOPIC_REF_MISSING");
      return null;
    }
    return clientState.topic_ref;
  };

  const requireThread = (): string | null => {
    if (!clientState.thread_ref) {
      reportBlock("CORE-01-ERR-THREAD-001:REQUIRED_THREAD_REF_MISSING");
      return null;
    }
    return clientState.thread_ref;
  };

  const requireWorkItem = (): string | null => {
    if (!clientState.work_item) {
      reportBlock("CORE-01-ERR-WORK-ITEM-001:REQUIRED_WORK_ITEM_MISSING");
      return null;
    }
    return clientState.work_item;
  };

  const runControl = (id: string) => {
    const action = actionUid(id);
    switch (action) {
      case "CORE-01-ACT-PROJECT-CREATE":
        void runServerAction(action);
        return;
      case "CORE-01-ACT-TOPIC-CREATE": {
        const projectId = requireProject();
        if (projectId) void runServerAction(action, { path_params: { projectId } });
        return;
      }
      case "CORE-01-ACT-THREAD-CREATE": {
        const projectId = requireProject();
        const work_item = requireWorkItem();
        if (projectId && work_item) void runServerAction(action, { path_params: { projectId }, payload: { work_item, ai_mode: clientState.ai_mode } });
        return;
      }
      ...
      case "CORE-01-ACT-SEND": {
        const conversationId = requireThread();
        if (!conversationId) return;
        if (!message.trim()) {
          reportBlock("CORE-01-ERR-CONVERSATION-001:MESSAGE_REQUIRED");
          return;
        }
        void runServerAction(action, {
          ...
          payload: {
            ...
            ai_mode: clientState.ai_mode,
            attachment_refs: clientState.attachment_refs,
            reference_refs: clientState.reference_refs,
          },
        });
        return;
      }
      case "CORE-01-ACT-CANDIDATE-CREATE":
        if (!humanDecision.trim()) {
          reportBlock("CORE-01-ERR-DECISION-001:HUMAN_DECISION_REQUIRED");
          return;
        }
        void runServerAction(action, { payload: { human_decision: humanDecision.trim(), evidence_refs: clientState.decision_evidence_refs } });
        return;
      case "CORE-01-ACT-CANDIDATE-ACCEPT":
      case "CORE-01-ACT-CANDIDATE-RETURN":
        reportBlock("CORE-01-ERR-CANDIDATE-001:EXACT_CANDIDATE_REF_REQUIRED");
        return;
      case "CORE-01-ACT-CANDIDATE-COMPARE":
        reportBlock("CORE-01-ERR-CANDIDATE-001:EXACT_CANDIDATE_SET_REQUIRED");
        return;
      case "CORE-01-ACT-PROJECT-VALIDATE": {
        const projectVersionId = requireProject();
        if (projectVersionId) void runServerAction(action, { path_params: { projectVersionId } });
        return;
      }
      case "CORE-01-ACT-PROJECT-CONFIRM": {
        const idValue = requireProject();
        if (idValue) void runServerAction(action, { path_params: { id: idValue } });
        return;
      }
      case "CORE-01-ACT-STORY-CANDIDATE": {
        const projectId = requireProject();
        if (projectId) void runServerAction(action, { path_params: { projectId } });
        return;
      }
      case "CORE-01-ACT-DNA-LOCK-REQUEST":
      case "CORE-01-ACT-CORE-REVIEW-SUBMIT":
      case "CORE-01-ACT-MOTHER-LOCK-REQUEST":
      case "CORE-01-ACT-CHILD-LOCK-REQUEST":
        if (!requireProject()) return;
        void runServerAction(action);
        return;
      case "CORE-01-ACT-BLUEPRINT-CREATE": {
        const idValue = requireTopic();
        if (idValue) void runServerAction(action, { path_params: { id: idValue } });
        return;
      }
      case "CORE-01-ACT-BLUEPRINT-VALIDATE":
      case "CORE-01-ACT-BLUEPRINT-APPROVE":
        reportBlock("CORE-01-ERR-BLUEPRINT-001:EXACT_BLUEPRINT_VERSION_REF_REQUIRED");
        return;
      case "CORE-01-ACT-CANONICAL-SCRIPT-VIEW": {
        const idValue = requireTopic();
        if (idValue) void runServerAction(action, { path_params: { id: idValue } });
        return;
      }
      case "CORE-01-ACT-ATTACHMENT":
        if (!requireThread()) return;
        reportBlock("CORE-01-ERR-UNDEFINED-001:GLOBAL_ATTACHMENT_SELECTOR_NOT_BOUND");
        return;
      case "CORE-01-ACT-REFERENCE-ATTACH":
        if (!requireThread()) return;
        reportBlock("CORE-01-ERR-UNDEFINED-001:GLOBAL_REFERENCE_SELECTOR_NOT_BOUND");
        return;
      default:
        reportBlock("CORE-01-ERR-UNDEFINED-001:ACTION_REQUIRES_EXACT_RUNTIME_CONTEXT");
    }
  };
  ...
  const runMessageMenu = (id: string) => {
    ...
    reportBlock(`${action}:EXACT_MESSAGE_REF_REQUIRED`);
  };
      ...
      <section className={styles.contextBar} data-section-id="CORE-01-SEC-01" data-visual-id="CORE-01-VIS-CONTEXT">
        <div className={styles.contextComponent} data-component-uid="CORE-01-CMP-CONTEXT">
          <label className={styles.selectField}>
            ...
            <select value={clientState.project_ref ?? ""} onChange={(event) => dispatchClient({ action_uid: "CORE-01-ACT-PROJECT-SELECT", project_ref: event.target.value || null })} data-control-id="CORE-01-CTL-PROJECT" data-action-uid={actionUid("CORE-01-CTL-PROJECT")} aria-label={t("core01.control.project")}><option value="">—</option></select>
          </label>
          ...
          <label className={styles.selectField}>
            ...
            <select value={clientState.topic_ref ?? ""} onChange={(event) => dispatchClient({ action_uid: "CORE-01-ACT-TOPIC-SELECT", topic_ref: event.target.value || null })} data-control-id="CORE-01-CTL-TOPIC" data-action-uid={actionUid("CORE-01-CTL-TOPIC")} aria-label={t("core01.control.topic")}><option value="">—</option></select>
          </label>
          ...
          <ReadonlyField id="CORE-01-FLD-NAMING-AUTHORITY" labelKey="core01.control.naming_authority" value="ACPOS_SYSTEM" />
        </div>
      </section>
          ...
          <section className={styles.panel} data-section-id="CORE-01-SEC-02" data-visual-id="CORE-01-VIS-LEFT">
            ...
            <div data-component-uid="CORE-01-CMP-THREADS">
              <div className={styles.threadHeader}>
                ...
                <ActionButton id="CORE-01-BTN-NEW-THREAD" labelKey="core01.control.new_thread" compact disabled={isBusy} onClick={() => runControl("CORE-01-BTN-NEW-THREAD")} />
              </div>
        ...
        <main className={styles.centerColumn}>
          <section className={`${styles.panel} ${styles.conversationHeader}`} data-section-id="CORE-01-SEC-03" data-visual-id="CORE-01-VIS-CENTER-HEADER">
            <div className={styles.conversationHeaderInner} data-component-uid="CORE-01-CMP-CONV-HEADER">
              ...
              <div className={styles.aiModeGroup}>
                <ActionButton id="CORE-01-BTN-SINGLE-AI" labelKey="core01.control.single_ai" compact disabled={isBusy} onClick={() => runControl("CORE-01-BTN-SINGLE-AI")} />
                <ActionButton id="CORE-01-BTN-MULTI-AI" labelKey="core01.control.multi_ai" compact disabled={isBusy} onClick={() => runControl("CORE-01-BTN-MULTI-AI")} />
              </div>
              <ReadonlyField id="CORE-01-FLD-ASSIGNED-AI" labelKey="core01.control.assigned_ai" />
              <ActionButton id="CORE-01-BTN-ASSISTANT-RECORD" labelKey="core01.control.assistant_record" compact disabled={isBusy} onClick={() => runControl("CORE-01-BTN-ASSISTANT-RECORD")} />
            </div>
          </section>
              ...
              {menuOpen && (
                <div className={styles.contextMenu} data-component-uid="CORE-01-CMP-MESSAGE-MENU" onClick={(event) => event.stopPropagation()}>
                  {MESSAGE_MENU.map((item) => (
                    <button key={item.id} type="button" className={styles.contextMenuItem} data-control-id={item.id} data-action-uid={actionUid(item.id)} data-runtime-binding="ACTION_BOUND" onClick={() => runMessageMenu(item.id)}>
                      {t(item.key)}
                    </button>
                  ))}
                </div>
            ...
            <div className={styles.decisionGrid}>
              ...
              <div className={styles.humanDecision} data-component-uid="CORE-01-CMP-HUMAN-DECISION">
                <label className={styles.textareaField}>
                  ...
                  <textarea data-control-id="CORE-01-FLD-HUMAN-DECISION" data-action-uid={actionUid("CORE-01-FLD-HUMAN-DECISION")} value={humanDecision} onChange={(event) => setHumanDecision(event.target.value)} aria-label={t("core01.control.human_decision")} />
                </label>
                <ReadonlyField id="CORE-01-FLD-STRUCTURED-DECISION" labelKey="core01.control.structured_decision" />
                <div className={styles.actionRow}>
                  <ActionButton id="CORE-01-BTN-CANDIDATE-CREATE" labelKey="core01.control.create_candidate" primary disabled={isBusy} onClick={() => runControl("CORE-01-BTN-CANDIDATE-CREATE")} />
                  <ActionButton id="CORE-01-BTN-CANDIDATE-CONFIRM" labelKey="core01.control.confirm_candidate" primary disabled={isBusy} onClick={() => runControl("CORE-01-BTN-CANDIDATE-CONFIRM")} />
                  <ActionButton id="CORE-01-BTN-RETURN-MODIFY" labelKey="core01.control.return_modify" disabled={isBusy} onClick={() => runControl("CORE-01-BTN-RETURN-MODIFY")} />
                </div>
              </div>
            </div>
          </section>
          ...
          <section className={styles.runtimeStrip} data-section-id="CORE-01-SEC-06" data-visual-id="CORE-01-VIS-RUNTIME">
            <div className={styles.runtimeComponent} data-component-uid="CORE-01-CMP-RUNTIME" data-control-id="CORE-01-FLD-RUNTIME-STAGE" data-action-uid={actionUid("CORE-01-FLD-RUNTIME-STAGE")}>
              <span>{t("core01.control.runtime_stage")}</span><strong>{runtimeDisplay}</strong>
            </div>
          </section>
          ...
          <section className={`${styles.panel} ${styles.composerPanel}`} data-section-id="CORE-01-SEC-07" data-visual-id="CORE-01-VIS-COMPOSER">
            <div className={styles.composer} data-component-uid="CORE-01-CMP-COMPOSER">
              <div className={styles.composerTools}>
                <ActionButton id="CORE-01-BTN-ATTACHMENT" labelKey="core01.control.attachment" compact disabled={isBusy} onClick={() => runControl("CORE-01-BTN-ATTACHMENT")} />
                <ActionButton id="CORE-01-BTN-REFERENCE" labelKey="core01.control.reference" compact disabled={isBusy} onClick={() => runControl("CORE-01-BTN-REFERENCE")} />
              </div>
              <textarea data-control-id="CORE-01-FLD-MESSAGE" data-action-uid={actionUid("CORE-01-FLD-MESSAGE")} aria-label={t("core01.control.message")} placeholder={t("core01.control.message")} value={message} onChange={(event) => setMessage(event.target.value)} />
              <ActionButton id="CORE-01-BTN-SEND" labelKey="core01.control.send" primary disabled={isBusy} onClick={() => runControl("CORE-01-BTN-SEND")} />
            </div>
          </section>
        </main>
        ...
        <aside className={styles.rightRail}>
          <section className={styles.panel} data-section-id="CORE-01-SEC-08" data-visual-id="CORE-01-VIS-RIGHT-CORE">
            <div className={styles.stateBlock} data-component-uid="CORE-01-CMP-PROJECT-STATE">
              ...
              <div className={styles.stackActions}>
                <ActionButton id="CORE-01-BTN-PROJECT-VALIDATE" labelKey="core01.control.project_validate" disabled={isBusy} onClick={() => runControl("CORE-01-BTN-PROJECT-VALIDATE")} />
                <ActionButton id="CORE-01-BTN-PROJECT-CONFIRM" labelKey="core01.control.project_confirm" disabled={isBusy} onClick={() => runControl("CORE-01-BTN-PROJECT-CONFIRM")} />
                <ActionButton id="CORE-01-BTN-STORY-CANDIDATE" labelKey="core01.control.story_candidate" disabled={isBusy} onClick={() => runControl("CORE-01-BTN-STORY-CANDIDATE")} />
                <ActionButton id="CORE-01-BTN-DNA-LOCK" labelKey="core01.control.dna_lock" disabled={isBusy} onClick={() => runControl("CORE-01-BTN-DNA-LOCK")} />
                <ActionButton id="CORE-01-BTN-CORE-REVIEW" labelKey="core01.control.core_review" disabled={isBusy} onClick={() => runControl("CORE-01-BTN-CORE-REVIEW")} />
                <ActionButton id="CORE-01-BTN-PROJECT-LOCK" labelKey="core01.control.project_lock" disabled={isBusy} onClick={() => runControl("CORE-01-BTN-PROJECT-LOCK")} />
              </div>
            </div>
            <div className={styles.divider} />
            <div className={styles.stateBlock} data-component-uid="CORE-01-CMP-BLUEPRINT-STATE">
              ...
              <div className={styles.stateValue}>—</div>
              <div className={styles.stackActions}>
                <ActionButton id="CORE-01-BTN-BLUEPRINT-CREATE" labelKey="core01.control.blueprint_create" disabled={isBusy} onClick={() => runControl("CORE-01-BTN-BLUEPRINT-CREATE")} />
                <ActionButton id="CORE-01-BTN-BLUEPRINT-VALIDATE" labelKey="core01.control.blueprint_validate" disabled={isBusy} onClick={() => runControl("CORE-01-BTN-BLUEPRINT-VALIDATE")} />
                <ActionButton id="CORE-01-BTN-BLUEPRINT-APPROVE" labelKey="core01.control.blueprint_approve" disabled={isBusy} onClick={() => runControl("CORE-01-BTN-BLUEPRINT-APPROVE")} />
                <ActionButton id="CORE-01-BTN-CHILD-LOCK" labelKey="core01.control.child_lock" disabled={isBusy} onClick={() => runControl("CORE-01-BTN-CHILD-LOCK")} />
              </div>
            </div>
          </section>
          ...
          <section className={styles.panel} data-section-id="CORE-01-SEC-09" data-visual-id="CORE-01-VIS-RIGHT-TOPIC">
            <div data-component-uid="CORE-01-CMP-TOPIC-PACKAGE">
              ...
              <ActionButton id="CORE-01-BTN-CANONICAL-SCRIPT" labelKey="core01.control.canonical_script" disabled={isBusy} onClick={() => runControl("CORE-01-BTN-CANONICAL-SCRIPT")} />
          ...
          <section className={styles.panel} data-section-id="CORE-01-SEC-10" data-visual-id="CORE-01-VIS-RIGHT-VERSION">
            <div data-component-uid="CORE-01-CMP-VERSION">
              ...
              <ActionButton id="CORE-01-BTN-CANDIDATE-COMPARE" labelKey="core01.control.candidate_compare" disabled={isBusy} onClick={() => runControl("CORE-01-BTN-CANDIDATE-COMPARE")} />
