...
export function QaCriteriaVisual(){
 ...
 return <div className={styles.page} data-page-uid="admin:SG-02" data-vis-step="VIS-16" data-authority-section-count="9" data-authority-control-count="11" data-page-state={projection?.page_state??"READ_ONLY"}>
  <section className={styles.contextBar} aria-label="SG-02 Context"><div className={styles.identity}><div className={styles.eyebrow}>ADMIN · SG-02 · QUALITY GOVERNANCE</div><h1>{qaCriteriaText(locale,"pageName")}</h1><p>{qaCriteriaText(locale,"pageRole")}</p></div><div className={styles.state}>QualityGovernance · FINAL_LOCKED · {runtimeError??projection?.page_state??"READ_ONLY"} · correlation {correlationId??"—"}</div></section>
