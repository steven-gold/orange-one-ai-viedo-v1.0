...
export function isStrategyRequestBuilderBound(){return builder!==null||isControlledTestMode();}
const CONTRACT={
...
export async function buildAndInvokeStrategyAction(input:StrategyFormalBuildInput){
 const controlledBuild=()=>{if(!isControlledTestMode())return null;const evidence_refs=['TEST-STR-EVIDENCE-001'];if(input.action_uid==='STR-01-ACT-SEND')return{path_params:{conversationId:input.conversation_id??''},payload:{page_uid:'workspace:STR-01',topic_ref:input.topic_ref,message:input.message,attachment_refs:[...input.attachment_refs],mode:input.mode}};if(input.action_uid==='STR-01-ACT-STOP')return{path_params:{conversationId:input.conversation_id??''},payload:{page_uid:'workspace:STR-01',topic_ref:input.topic_ref,mode:input.mode}};if(input.action_uid==='STR-01-ACT-COMPARE')return{query:{candidate_refs:(input.compare_candidate_refs.length?input.compare_candidate_refs:['TEST-STR-CANDIDATE-001','TEST-STR-CANDIDATE-002']).join(',')}};if(input.action_uid==='STR-01-ACT-REVIEW')return{payload:{page_uid:'workspace:STR-01',candidate_ref:input.candidate_ref,candidate_version_ref:input.candidate_version_ref,analysis_basis:'TEST_ONLY exact source refs',risk:'TEST_ONLY capacity risk',uncertainty:'TEST_ONLY external confidence 0.82',evidence_refs}};return{payload:{page_uid:'workspace:STR-01',candidate_ref:input.candidate_ref,candidate_version_ref:input.candidate_version_ref,human_review:{decision:'APPROVED',reason:'TEST_ONLY controlled human approval fixture'},evidence_refs}};};
 if(!builder&&!isControlledTestMode())return{ok:false as const,error_uid:errorUid(input.action_uid),reason_code:'STRATEGY_REQUEST_ADAPTER_NOT_BOUND',correlation_id:'unresolved'};
 let built:Awaited<ReturnType<StrategyRequestBuilder['build']>>;try{const controlled=builder?null:controlledBuild();built=builder?await builder.build(input):(controlled??{});}catch{return{ok:false as const,error_uid:errorUid(input.action_uid),reason_code:'STRATEGY_REQUEST_ADAPTER_FAILED',correlation_id:'unresolved'};}
