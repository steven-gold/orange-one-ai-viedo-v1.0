...
export function buildControlledAssetRequest(input:{action_uid:AssetActionUid;control_uid:string;control_value?:unknown;state:Readonly<AssetClientState>;projection:AssetNormalizedProjection|null;correction_request:string}){
 ...
 switch(input.action_uid){
  case"ASSET-01-ACT-FLOW-START":case"ASSET-01-ACT-TASK-RETRY":case"ASSET-01-ACT-CORRECTION-EXECUTE":if(!taskId)throw new Error("TASK_ID_REQUIRED");return{path_params:{taskId},payload:{...common,mode:input.state.mode}};
...
export function prepareControlledAssetEvaluation(input:AssetInvokeInput){void input;const p=lastProjection;if(!p?.task_id||!p.output_version_id)throw new Error("EVALUATION_CONTEXT_REQUIRED");return{test_metadata:meta(),target_output_version_id:p.output_version_id,criteria_version_id:p.values["ASSET-01-FLD-CRITERIA"],dimensions:[{dimension_uid:"identity",score:98},{dimension_uid:"continuity",score:97},{dimension_uid:"technical",score:99}],overall_score:98,hard_block:false,evidence_refs:[p.output_version_id]};}
export function prepareControlledCorrectionExecution(input:AssetInvokeInput){void input;const p=lastProjection;if(!p?.task_id||!p.output_version_id||!approvedCorrectionRef)throw new Error("APPROVED_CORRECTION_REQUIRED");return{test_metadata:meta(),mode:"CORRECTION",source_asset_version_id:p.output_version_id,approved_candidate_ref:approvedCorrectionRef,translated_instruction_ref:createControlledTestRef("ASSET-CORRECTION-INSTRUCTION"),affected_scope:"CURRENT_ASSET_ONLY"};}

export async function controlledAssetSharedInvoke(operation_id:string,input:AssetInvokeInput):Promise<AssetInvokeResult>{
 void input;if(!isControlledAssetClientTestMode())return{ok:false,error_uid:"ASSET-01-ERR-CONTEXT-001",reason_code:"ASSET_TEST_RUNTIME_DISABLED",correlation_id:"unresolved"};
