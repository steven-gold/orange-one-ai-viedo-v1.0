...
export type DbClientResult={ok:true;value:unknown;correlation_id:string}|{ok:false;error_uid:DbErrorUid;reason_code:string;correlation_id:string};
export type DbClientBindings={readProjection:()=>Promise<{ok:true;projection:DbNormalizedProjection;correlation_id:string}|{ok:false;error_uid:DbErrorUid;reason_code:string;correlation_id:string}>;read:(port_uid:DbReadPortUid,input:DbReadInput)=>Promise<DbClientResult>;resolveSystemLifecycle:()=>Promise<{ok:true;href:string}|{ok:false;reason_code:string}>};
...
export async function readDbProjection(){if(!binding)return{ok:false as const,error_uid:'DB-01-ERR-CONTEXT-001' as const,reason_code:'DB_CLIENT_RUNTIME_NOT_BOUND',correlation_id:'unresolved'};try{return await binding.readProjection();}catch{return{ok:false as const,error_uid:'DB-01-ERR-CONTEXT-001' as const,reason_code:'DB_PROJECTION_READ_FAILED',correlation_id:'unresolved'};}}
