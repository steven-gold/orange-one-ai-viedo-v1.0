...
BASE_HEAD="56e176cc30f43b8c3f9d021bee173103669de483"
...
S02=SYSTEM_DOCS[1];S03=SYSTEM_DOCS[2];S04=SYSTEM_DOCS[3];S08=SYSTEM_DOCS[7];S09=SYSTEM_DOCS[8]
PAIR_PRECEDENCE={
...
for page,m in page_maps.items():
    for uid,r in m.items():
        ...
        for f in fs:
            remaining.append({"field":f,"page":page,"uid":uid,"label":r.get("label",""),"operation":op,
              "runtime_status":st,"runtime_owner":r.get("runtime_owner",""),"method_path":r.get("method_path",""),
              "payload_schema":r.get("payload_schema",""),"persistence_owner":r.get("persistence_owner",""),
              "action":r.get("action",""),"gate":r.get("gate",""),"permission":r.get("permission",""),
              "canonical_owner":owner})

assert len(remaining)==239,len(remaining)
sys06=[x for x in remaining if x["canonical_owner"]==S06]
assert len(sys06)==49,len(sys06)

controls={}
for x in sys06:
    key=(x["page"],x["uid"])
    controls.setdefault(key,{"page":x["page"],"uid":x["uid"],"label":x["label"],"operation":x["operation"],
...
# Gather owner evidence using operation vocabulary + domain keywords.
keywords=ops+[
"enterprise","development","candidate","campaign","discovery","outreach","email","lead","company","business",
"erp","finance","forecast","capacity","projection","guardrail","revenue","cost","budget","invoice","payment",
"connector","sync","import","export","mapping","record","persistence","audit","evidence","approval","merge",
"pause","resume","start","stop","dispatch","send","read model","projection","external"
]
rx=re.compile("|".join(re.escape(x) for x in sorted(set(keywords),key=len,reverse=True)),re.I)
owner_doc=Document(S06)
evidence={"paragraphs":[],"rows":[]}
for i,p in enumerate(owner_doc.paragraphs):
    t=norm(p.text)
    if t and rx.search(t):evidence["paragraphs"].append({"index":i,"text":t[:4500]})
for ti,t in enumerate(owner_doc.tables):
    if not t.rows:continue
    h=[norm(c.text) for c in t.rows[0].cells]
    for ri,row in enumerate(t.rows[1:],2):
        vals=[norm(c.text) for c in row.cells];joined=" | ".join(vals)
        if joined and rx.search(joined):
            evidence["rows"].append({"table":ti+1,"row":ri,"headers":h,"values":vals,"joined":joined[:7500]})

owner_rows=parse_controls(S06)
owner_exact={}
for key,c in controls.items():
    rr=[r for r in owner_rows if r["control"]==c["uid"]]
    owner_exact[f"{c['page']}::{c['uid']}"]=rr

by_page=collections.Counter(x["page"] for x in sys06)
by_field=collections.Counter(x["field"] for x in sys06)
by_status=collections.Counter(x["runtime_status"] for x in sys06)
by_runtime_owner=collections.Counter(x["runtime_owner"] for x in sys06)
by_operation=collections.Counter(x["operation"] for x in sys06)
...
"by_runtime_status":dict(by_status),
"by_runtime_owner":dict(by_runtime_owner),
"by_operation":dict(by_operation),
"owner_paragraph_hits":len(evidence["paragraphs"]),
"owner_table_hits":len(evidence["rows"]),
}
report={
"marker":"ACPOS-20260922-BATCH-48-SYSTEM06-ENTERPRISE-ERP-CONTRACT-DESIGN-AUDIT-V1",
"base_head":BASE_HEAD,
"summary":summary,
"operations":ops,
"controls":list(controls.values()),
"owner_exact_rows":owner_exact,
"owner_evidence":evidence,
"system06_cells":sys06,
}
Path("__batch48_system06_contract_audit.json").write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
Path("__batch48_summary.txt").write_text(json.dumps({"summary":summary,"controls":list(controls.values())},ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
print("BATCH48_SUMMARY="+json.dumps(summary,ensure_ascii=False,sort_keys=True))
