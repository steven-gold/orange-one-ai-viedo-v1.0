...
def load(p):
    ...
    return o
d=load(DOC)
if d.get('artifact_type')!='NON_NORMATIVE_STAGE02_TRANSITION_LEDGER_REQUIRED_APPLICABILITY_R34' or d.get('stage_uid')!='STAGE-02' or d.get('normative_authority') is not False: die('R34_IDENTITY_DRIFT')
...
if den.get('not_applicable_count')!=0 or den.get('optional_count')!=0 or den.get('required_count')!=40 or den.get('candidate_values_materialized')!=0 or den.get('blocker_reduction_claimed')!=0: die('R34_APPLICABILITY_RESULT_DRIFT')
...
if len({(x.get('scope'),x.get('transition_uid')) for x in rows})!=10: die('R34_TRANSITION_SET_DRIFT')
if Counter((x.get('scope'),x.get('transition_uid')) for x in rows)!=Counter({k:4 for k in {(x.get('scope'),x.get('transition_uid')) for x in rows}}): die('R34_TRANSITION_FOUR_FIELD_MATRIX_DRIFT')
...
for x in rows:
    ...
    if x.get('illegal_transition_tests_present') is not True: die(f'R34_ILLEGAL_TRANSITION_TEST_BASELINE_DRIFT:{x.get("transition_uid")}')
...
print('PASS: R34 validates 10 transitions x 4 REQUIRED ledger fields under WEB-GOV-01-S060; no N/A/optional/materialization')
