...
def write(p,d): p.write_text(yaml.safe_dump(d,allow_unicode=True,sort_keys=False),encoding='utf-8')
...
with tempfile.TemporaryDirectory() as td:
    ...
    cur=load(t/'GOVERNANCE_CURRENT.yaml'); na=cur.setdefault('normative_authority',{})
    na['version']='v2.1.6'; na['package_sha256']=V216_PKG; na['external_trust_root_sha256']=V216_TRUST
    write(t/'GOVERNANCE_CURRENT.yaml',cur)
    base=load(t/'REBUILD_BRANCH_BASELINE.yaml'); base.setdefault('governance_test',{})['version']='v2.1.6'; write(t/'REBUILD_BRANCH_BASELINE.yaml',base)
    lockp=t/'11_EVIDENCE/audit/GOVERNANCE_STAGE_LOCK.yaml'; lock=load(lockp); lock.setdefault('current_test_authority',{})['version']='v2.1.6'; write(lockp,lock)
    sealp=t/'11_EVIDENCE/audit/SEALED_GOVERNANCE_TEST_BASELINE.yaml'; seal=load(sealp); seal.setdefault('sealed_governance',{})['version']='v2.1.6'; write(sealp,seal)
    ...
    if (projected_cur.get('version'),projected_cur.get('package_sha256'),projected_cur.get('external_trust_root_sha256')) != ('v2.1.6',V216_PKG,V216_TRUST): die('isolated v2.1.6 authority projection self-check failed')
    if projected_base.get('version')!='v2.1.6' or projected_lock.get('version')!='v2.1.6' or projected_seal.get('version')!='v2.1.6': die('isolated v2.1.6 pointer projection self-check failed')
    ...
    # Historical Source-Facts validator also binds RUN_MANIFEST.governance.version.
    # Project that single historical field only inside the temporary workspace.
    rmp=run/'RUN_MANIFEST.yaml'; rm=load(rmp); rm.setdefault('governance',{})['version']='v2.1.6'; write(rmp,rm)
    if load(rmp).get('governance',{}).get('version')!='v2.1.6': die('isolated v2.1.6 RUN_MANIFEST projection self-check failed')

    sp=run/'EXECUTION_STATE.yaml'; state=load(sp)
    ...
    for script in (root/'governance/ci/validate_current_v216_workspace.py',root/'governance/ci/validate_current_v216_source_facts.py'):
        ...
        if r.returncode!=0:
            print(r.stdout); print(r.stderr,file=sys.stderr); die('historical upstream projection validator failed: '+script.name)
