# R2_TRIGGER: fresh CORE-01 Stage-01 Stage-02 replay retry-3
#!/usr/bin/env python3
...
def reset_current_state_for_stage1():
    ...
    for k in list(state):
        ...
        if kl.startswith('stage02') or kl.startswith('stage2_') or k in {
          'work_unit_resolution_gate','last_closed_product_work_unit','active_work_unit',
...
def stage2_structural_materialize(stage1_meta, initial):
    ...
    for c in controls.values():
        controls_by_action[c.get('action_uid')].append(c)
    ...
    dump(page_dir/'BUSINESS_ENTITY_INVENTORY.yaml',{
      ...
      'derivation':'EXACT_OBJECT_REGISTRY_PROJECTION_NO_NEW_ENTITY','entity_count':len(objects),
      'entities':[{'object_uid':k,'entity':v.get('entity'),'owner':v.get('owner'),'required_content':v.get('required_content')} for k,v in objects.items()],
    ...
    dump(page_dir/'BUSINESS_ENTITY_OPERATION_MATRIX.yaml',{
      ...
      'derivation':'EXACT_ACTION_CONTROL_PORT_GATE_PERMISSION_PROJECTION','semantic_entity_join_used':False,
      'operation_count':len(actions),'operations':[{
        'action_uid':aid,'label':a.get('label'),'effect_type':a.get('effect_type'),'gate_uid':a.get('gate_uid'),
        'permission_uid':a.get('permission_uid'),'runtime_binding':a.get('runtime_binding'),
    ...
    dump(page_dir/'ENTITY_HIERARCHY_MATRIX.yaml',{
      ...
      'derivation':'EXACT_OBJECT_PROJECTION_NO_PARENT_INFERENCE','rows':[{
        'object_uid':k,'entity':v.get('entity'),'owner':v.get('owner'),'parent_object_uid':None,
    ...
    dump(page_dir/'FUNCTIONAL_WORKBENCH_CONTRACT.yaml',{
      ...
      'derivation':'EXACT_SECTION_COMPONENT_CONTROL_MEMBERSHIP','workbenches':[{
    ...
    for cid,c in controls.items():
        for field,rel in [('component_uid','COMPONENT_HAS_CONTROL'),('action_uid','CONTROL_TRIGGERS_ACTION'),('gate_uid','CONTROL_REQUIRES_GATE'),('permission_uid','CONTROL_REQUIRES_PERMISSION'),('visual_uid','CONTROL_RENDERED_IN_VISUAL')]:
            if c.get(field): edges.append({'from':cid if field!='component_uid' else c.get(field),'to':c.get(field) if field!='component_uid' else cid,'relation':rel})
    dump(page_dir/'INTERACTION_TOPOLOGY_SPEC.yaml',{
      ...
      'derivation':'EXACT_REGISTRY_UID_EDGES_ONLY','semantic_edge_inference_used':False,'edge_count':len(edges),'edges':edges,
    })
    dump(page_dir/'FUNCTION_VISUAL_IMPACT_MATRIX.yaml',{
      ...
      'derivation':'EXACT_CONTROL_ACTION_VISUAL_BINDING','rows':[{
        'action_uid':aid,'bindings':[{
          'control_uid':c.get('control_uid'),'section_uid':c.get('section_uid'),'component_uid':c.get('component_uid'),'visual_uid':c.get('visual_uid')
        } for c in controls_by_action.get(aid,[])]
      } for aid in actions],
    })
    if isinstance(raw.get('conversation_policy'),dict):
        dump(page_dir/'AI_INTERACTION_CONTINUITY_CONTRACT.yaml',{
          ...
          'derivation':'EXACT_CONVERSATION_POLICY_PROJECTION','conversation_policy':raw.get('conversation_policy'),
    ...
    dump(page_dir/'PAGE_CONSTRUCTION_SPEC_PACKAGE.yaml',{
      ...
      'source_blueprint_uid':bp.get('blueprint_uid'),
      'included_artifacts':[
        ...
        'AI_INTERACTION_CONTINUITY_CONTRACT.yaml','FUNCTIONAL_CHAIN_SPEC.yaml'
      ],'external_authority_resolution_performed':False,'ai_autofill_used':False,'stage_exit_claimed':False,
    })
    dump(out/'DEPENDENCY_MAP.yaml',{
      'schema_version':1,'artifact_type':'DEPENDENCY_MAP','stage_uid':'STAGE-02',
      'pages':[{'page_uid':PAGE,'blueprint_uid':bp.get('blueprint_uid'),'blueprint_hash':bp.get('blueprint_hash'),'unresolved_external_authority_refs':refs}],
    ...
    for g in gaps:
        ...
        score.append({'gap_uid':stable_uid('PROBLEM',PAGE,g.get('class'),g.get('category'),g.get('uid'),g.get('detail')),
                      'source_gap':g,'admission':'REQUIRED','route':route})
        ledger.append({'gap_uid':score[-1]['gap_uid'],'route':route,'auto_completion_allowed':False,'reason':'FRESH_SOURCE_HAS_NO_EXACT_PREEXISTING_FIELD_CLOSURE'})
    dump(page_dir/'FUNCTION_ADMISSION_SCORECARD.yaml',{
      'schema_version':1,'artifact_type':'FUNCTION_ADMISSION_SCORECARD','stage_uid':'STAGE-02','page_uid':PAGE,
      'fresh_gap_count':len(score),'rows':score,'product_blocker_credit':0,
    })
    dump(page_dir/'AUTO_COMPLETION_SCOPE_LEDGER.yaml',{
      'schema_version':1,'artifact_type':'AUTO_COMPLETION_SCOPE_LEDGER','stage_uid':'STAGE-02','page_uid':PAGE,
      'fresh_gap_count':len(ledger),'rows':ledger,'automatic_product_materialization_performed':False,'product_blocker_credit':0,
    })
...
def materialize_stage2_projection(final):
    ...
    ex['stage2']={
      'result':'TEST_EXECUTED_BLOCKED' if problems else 'PASS',
    ...
    state['status']='ACTIVE_STAGE2_TESTED_BLOCKED_FRESH_CORE01' if problems else 'ACTIVE_STAGE2_CLOSED_FRESH_CORE01'
...
def main():
    ...
    print(json.dumps({
      ...
      'stage1':'PASS','stage2_result':final.get('result'),
