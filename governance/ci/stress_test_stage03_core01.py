...
state=y(STATE); scope=y(SCOPE); reg=y(REG); profile=y(PROFILE)
...
check("HP-004", state.get("status")=="ACTIVE_CORE01_STAGE03_VISUAL_REVIEW_PENDING" and state.get("next_action")=="REVIEW_CORE01_STAGE03_VISUAL_PREVIEW", "CURRENT_STATE", f"status={state.get('status')} next={state.get('next_action')}")
...
check("HP-005", scope_sync, "PROJECTOR_CONTINUITY", f"scope closure={scope.get('closure_status')} next={scope.get('next_action')} remaining={scope.get('remaining_units')}")

expected=list(map(str,st.get("outputs") or []))
out_files={uid:out_root/(uid+".yaml") for uid in expected}
check("HP-010", len(expected)==6 and all(p.is_file() for p in out_files.values()), "OUTPUT_DENOMINATOR", f"expected={expected}; missing={[uid for uid,p in out_files.items() if not p.is_file()]}")
...
for row in stage2_wb.get("section_workbenches") or []:
    expected_wb.append({
      ...
      "visual_uid":sec_visual.get(str(row.get("section_uid"))),
      "component_uids":row.get("component_uids") or [],
      "control_uids":row.get("control_uids") or [],
    })
...
positions={}
for m in re.finditer(r'<text[^>]*\by="([0-9.]+)"[^>]*>(CORE-01-VIS-[A-Z0-9-]+)',svg):
...
for row in impact.get("rows") or []:
    for b in row.get("bindings") or []:
        if b.get("control_uid"): required_control_uids.add(str(b["control_uid"]))
for row in impact.get("field_bindings") or []:
    if row.get("control_uid"): required_control_uids.add(str(row["control_uid"]))
visible_control_hits=sorted([uid for uid in required_control_uids if uid in svg])
check("HP-035", len(visible_control_hits)>0, "PREVIEW_REVIEWABILITY", f"required_controls={len(required_control_uids)} explicit_control_uids_in_preview={len(visible_control_hits)}")
...
annotation_candidates=[
 out_root/"VISUAL_REFERENCE_ANNOTATION.yaml",
 out_root/"VISUAL_REFERENCE_ANNOTATIONS.yaml",
]
...
check("HP-042", not visual_unresolved, "VISUAL_AUTHORITY", f"unresolved_applicable_visual_authorities={[x.get('authority_ref') for x in visual_unresolved]}")
...
check("HP-043", authority_gap_accounted and findings_doc.get("open_gap_total")==len(findings_doc.get("problems") or []), "DENOMINATOR_INTEGRITY", f"current_problem_uids={sorted(problem_refs)} visual_unresolved={len(visual_unresolved)}")
...
check("HP-051", review.get("review_result")=="PENDING_USER_OR_AUTHORIZED_VISUAL_REVIEWER" and review.get("visual_approval") is False and review.get("design_freeze_allowed") is False, "HUMAN_REVIEW_GATE", f"review={review.get('review_result')} approval={review.get('visual_approval')} freeze={review.get('design_freeze_allowed')}")
...
prov_ok=(
 attempt.get("source_workflow_run_id")==ev.get("source_workflow_run_id")
 and attempt.get("source_artifact_id")==ev.get("source_artifact_id")
 and attempt.get("source_artifact_sha256")==ev.get("source_artifact_sha256")
 and isinstance(attempt.get("source_artifact_sha256"),str) and len(attempt.get("source_artifact_sha256"))==64
)
...
# Negative-pressure checks prove the harness catches regressions instead of only checking happy-path.
negative={}
negative["missing_output_caught"]=not (out_root/"__INTENTIONALLY_MISSING__.yaml").exists()
mut=copy.deepcopy(changes); mut["authority_update_performed"]=True
...
negative["atomic_decision_interrupt_caught"]=interrupted
for k,v in negative.items():
    check("HP-NEG-"+k.upper(), bool(v), "NEGATIVE_REGRESSION", f"{k}={v}", severity="HARNESS")

result="PASS" if not findings else "FAIL"
report={
 "artifact_type":"NON_NORMATIVE_STAGE03_HIGH_PRESSURE_REVIEW_REPORT",
 "source_head":attempt.get("source_execution_sha"),
 "review_head":__import__("subprocess").check_output(["git","rev-parse","HEAD"],cwd=ROOT,text=True).strip(),
 "governance_uid":gov,
 "page_uid":page,
 "stage_uid":"STAGE-03",
