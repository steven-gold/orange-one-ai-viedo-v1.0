...
def txt(d): return yaml.safe_dump(d,allow_unicode=True,sort_keys=False)
def add(gaps,page,klass,category,uid,detail,owner='PAGE_FUNCTIONAL_CONTRACT'): gaps.append({'page':page,'class':klass,'category':category,'uid':uid,'detail':detail,'gap_owner':owner})
def port_refs(rb):
 out=[]
 for k in ('port_uid','persist_via_port_uid','execute_port_uid','decision_port_uid'):
  v=rb.get(k) if isinstance(rb,dict) else None
  if v: out.append((k,v))
 return out
def port_operation(p): return (p or {}).get('operation') or (p or {}).get('registered_operation')
def port_method(p): return (p or {}).get('method_path') or (p or {}).get('method_effective_path')
def port_permission(p): return (p or {}).get('permission') or (p or {}).get('registered_permission')
def state_event(p): return str((p or {}).get('state_event') or '')
def event_token(se):
 if '|' in se:
...
def has_state_transition(se): return '→' in se or '->' in se
def has_explicit_validation(a,rb,p):
 return fieldish(a,'success_contract','validation_contract','validator_uid') or fieldish(rb,'validation','validation_rule','evaluation_rule') or fieldish(p,'validation','validation_rule','validator_uid')
def has_payload_contract(a,rb,p):
 if fieldish(a,'payload','payload_rule','input_contract','request_contract'): return True
 if fieldish(rb,'payload','payload_rule','payload_mode','input_contract','request_contract'): return True
 if fieldish(p,'payload','payload_rule','input_contract','request_contract','request_schema'): return True
 return False
...
for page,raw_path in PAGES.items():
 ...
 gaps=[]; refs=[]; controls_by_action=defaultdict(list); transitions_by_action=defaultdict(list)
 ...
 for aid,a in actions.items():
  ...
  elif not any((trans.get(tid) or {}).get('recovery') for tid in transitions_by_action.get(aid,[])):
  ...
  if not controls_by_action.get(aid) and not explicit_trigger: add(gaps,page,'ARCHITECTURE_GAP','ACTION_WITHOUT_CONTROL_OR_TRIGGER',aid,'no registered control or exact transition/system trigger')
  ...
  if client:
   ...
   if not auth or not op: add(gaps,page,'IMPLEMENTATION_GAP','SHARED_OWNER_REFERENCE_INCOMPLETE',aid,str((auth,op)))
    ...
    if auth in KNOWN_GAPS: add(gaps,page,'AUTHORITY_GAP','SHARED_OWNER_AUTHORITY_UNRESOLVED',aid,KNOWN_GAPS[auth]+': '+auth,owner='EXTERNAL_AUTHORITY')
  else:
   if not resolved_ports: add(gaps,page,'IMPLEMENTATION_GAP','RUNTIME_ENTRY_OR_PORT_MISSING',aid,str(kind))
   for puid,p in resolved_ports:
    if not port_operation(p): add(gaps,page,'IMPLEMENTATION_GAP','PORT_OPERATION_MISSING',aid,puid)
    if not port_method(p): add(gaps,page,'IMPLEMENTATION_GAP','API_ENTRY_MISSING',aid,puid)
    if not port_permission(p): add(gaps,page,'IMPLEMENTATION_GAP','PORT_PERMISSION_MISSING',aid,puid)
    refs.append({'page':page,'action_uid':aid,'classification':'REGISTERED_PORT_REFERENCE','binding_kind':kind,'port_uid':puid,'operation':port_operation(p),'api_entry':port_method(p)})
  if is_effectful and kind!='SHARED_OPERATION_REFERENCE':
   p=(resolved_ports[0][1] if resolved_ports else {})
   if not has_explicit_validation(a,rb,p): add(gaps,page,'ARCHITECTURE_GAP','POST_ACTION_VALIDATION_NODE_MISSING',aid,'no explicit success/validation/evaluation contract; provenance decision/result is not validation')
   if not has_payload_contract(a,rb,p): add(gaps,page,'INPUT_SOURCE_GAP','PAYLOAD_INPUT_CONTRACT_MISSING',aid,'registered operation exists but no explicit payload/input contract or schema')
   se=state_event(p)
   if not event_token(se): add(gaps,page,'ARCHITECTURE_GAP','AUDIT_EVENT_NODE_MISSING',aid,'registered port has no explicit audit/event UID in state_event')
   if not (has_state_transition(se) or bool(transitions_by_action.get(aid)) or a.get('state_effect')): add(gaps,page,'ARCHITECTURE_GAP','SUCCESS_NEXT_STATE_BINDING_MISSING',aid,'no state_effect, state transition, or port state_event transition')
  if kind!='SHARED_OPERATION_REFERENCE':
   body=txt({'action':a,'runtime':rb})
   for ref,gid in KNOWN_GAPS.items():
    if ref in body: add(gaps,page,'AUTHORITY_GAP','KNOWN_EXTERNAL_AUTHORITY_DEPENDENCY',aid,gid+': '+ref,owner='EXTERNAL_AUTHORITY')
 for tid,t in trans.items():
  ...
  for req in ('mutation_owner','failure_state','recovery','audit_event_uid','illegal_transition_tests'):
   if t.get(req) in (None,'',[],{}): add(gaps,page,'ARCHITECTURE_GAP','STATE_TRANSITION_LEDGER_FIELD_MISSING',tid,req)
 uniq=[]; seen=set()
 for g in gaps:
 ...
 all_summary[page]=summary; all_gaps.extend(gaps); all_refs.extend(refs)
affected_authority_consumers=defaultdict(list)
for g in all_gaps:
 if g['class']=='AUTHORITY_GAP':
  m=re.search(r'(GAP-\d{3})',g['detail'])
  affected_authority_consumers[m.group(1) if m else g['detail']].append({'page':g['page'],'uid':g['uid'],'category':g['category']})
result={'schema_version':2,'detector_revision':'v3-authority-native-semantic-calibrated','run_uid':'FRESH-RUN-003','stage_uid':'STAGE-02','operation_uid':'FUNCTIONAL_CHAIN_COMPILE','governance_overlay':'v2.1.12','summary':all_summary,'gap_total':len(all_gaps),'gaps':all_gaps,'registered_runtime_references':all_refs,'unique_external_authority_gaps':dict(sorted(affected_authority_consumers.items())),'functional_completion':len(all_gaps)==0,'semantics':'PASS means exhaustive scan/classification only; gap_total>0 blocks functional completion'}
...
print('STAGE2_UNIQUE_EXTERNAL_AUTHORITY_GAPS='+json.dumps(dict(sorted(affected_authority_consumers.items())),ensure_ascii=False,sort_keys=True))
