...
def load(path: Path):
    ...
    except Exception as exc: die(f'PARSE:{path.relative_to(ROOT)}:{exc!r}')
...
def git(*args: str) -> str:
    ...
    if cp.returncode != 0: die(f'GIT_FAILED:{args!r}:{cp.stderr.strip()}')
...
if (execution.get('stage2') or {}).get('result') != 'NOT_EXECUTED': die('ACTIVE_STATE_STAGE2_NOT_NOT_EXECUTED')
...
if active_attempt_state_key or state.get('stage02_active_attempt'):
    die('NOT_EXECUTED_STAGE_MUST_NOT_PERSIST_ACTIVE_ATTEMPT_POINTER')
