...
common_always = ['04_PAGE_FUNCTIONAL_CONTRACT/DEPENDENCY_MAP.yaml','04_PAGE_FUNCTIONAL_CONTRACT/SHARED_OWNER_PORT_MAP_R2.yaml']
...
def add_missing(blockers, path, code, applies=True, basis=None, aliases=()):
    ...
    if not any(p.exists() for p in [path, *aliases]):
        row={'code':code,'class':'MISSING_APPLICABLE_REQUIRED_OUTPUT','path':str(path),'applicability':'APPLIES'}
        if basis: row['applicability_basis']=basis
        blockers.append(row)
...
def page_result(page):
    ...
    add_missing(blockers,page_dir/'FUNCTION_VISUAL_IMPACT_MATRIX.yaml','MISSING_FUNCTION_VISUAL_IMPACT_MATRIX',True,'USER_VISIBLE_PAGE_OPERATIONS')
    ...
    if stage1!='PASS': blockers.insert(0,{'code':'STAGE1_NOT_CLOSED','class':'PREREQUISITE','missing':s1miss})
    return {
      'page_uid':page,'stage1_result':stage1,'stage1_missing':s1miss,
      'stage2_result':'PASS' if not blockers else 'BLOCKED','stage2_blocker_count':len(blockers),'stage2_blockers':blockers,
      'applicability':{'async_provider':async_applies,'business_entity':entity_applies,'entity_hierarchy':hierarchy_applies,
        'continuous_workbench':workbench_applies,'function_visual_impact':True,'function_admission':admission_activation,
        'bounded_auto_completion':auto_activation,'ai_interaction_continuity':ai_applies}}

resolved=resolve()
manifest=(ROOT/resolved['specification_manifest']).read_text(encoding='utf-8')
for token in ('applicability_must_be_proven_before_missing_output_is_counted_as_blocker: true','GOV-COMP-TEST-FEEDBACK-TEMPORARY-ARTIFACT-LIFECYCLE'):
    if token not in manifest:
        print('ERROR: active specification manifest missing guard: '+token,file=sys.stderr); sys.exit(2)
results=[page_result(p) for p in PAGES]
out={'schema_version':3,'run_uid':'FRESH-RUN-003-TARGETED-STAGE02-RERUN','branch':os.getenv('GITHUB_REF_NAME','LOCAL'),
 'scope_pages':PAGES,'governance_uid':resolved['governance_uid'],'display_version':resolved['display_version'],
 'specification_runtime_sha256':resolved['runtime_bundle_sha256'],'applicability_model':'EXACT_APPLICABILITY_BEFORE_BLOCKER_COUNT',
 'results':results,'engine_result':'PASS','closure_claim':all(r['stage2_result']=='PASS' for r in results)}
out_path=RUN/'04_PAGE_FUNCTIONAL_CONTRACT/TARGETED_STAGE2_RERUN_RESULT.json'
