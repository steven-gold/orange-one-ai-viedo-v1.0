...
The successor execution-binding denominator MUST be mechanically derived from the Current lifecycle operation set and Current Authority/applicability; it MUST NOT be a consumer-local hardcoded product stack. A requirement MAY resolve as `AUTHORIZED_NOT_APPLICABLE` only with exact Authority evidence. Technology, repository, runtime, database, authentication, authorization, provider, environment, test, build, deployment, monitoring, backup, rollback, or next-unit target values MUST NOT be created merely because the downstream Stage needs them. Implementation language, frontend/backend framework, package/dependency manager, compiler/transpiler or equivalent execution-toolchain choices are execution Authority when they materially constrain generated program artifacts; if not already bound by Current Authority, the successor is BLOCKED/REENTER rather than AI-recommended or locally invented.

For the standard lifecycle, the existing handoff gate MUST at minimum reconcile these capability classes when applicable:
- implementation admission: repository/application target and owner, implementation language, frontend framework, backend framework, package/dependency manager authority, frontend/runtime, backend/runtime, data-access/database, authentication/authorization/security/audit, external integration, async runtime and storage;
