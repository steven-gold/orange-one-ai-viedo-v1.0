...
TARGET=ROOT/'governance/ci/run_current_stage3_visual_design.py'
...
def preview_svg(page_auth, workbench, impact):
    ...
    positions={
      'CORE-01-VIS-CONTEXT':70,
      'CORE-01-VIS-LEFT':170,
      'CORE-01-VIS-CENTER-HEADER':180,
      'CORE-01-VIS-MESSAGES':320,
      'CORE-01-VIS-RUNTIME':520,
      'CORE-01-VIS-COMPOSER':620,
      'CORE-01-VIS-DECISION':760,
      'CORE-01-VIS-RIGHT-CORE':180,
      'CORE-01-VIS-RIGHT-TOPIC':420,
      'CORE-01-VIS-RIGHT-VERSION':660,
    }
    ...
    parts=[
      ...
      '<text x="20" y="24">CORE-01 STAGE-03 AUTHORITY-BOUNDED REVIEWABILITY PREVIEW</text>',
      '<text x="20" y="44">GLOBAL VISUAL / SHELL AUTHORITY UNRESOLVED — HUMAN VISUAL REVIEW NOT REACHED</text>',
    ]
    boxes={
      'CORE-01-VIS-CONTEXT':(20,50,1240,54),
      'CORE-01-VIS-LEFT':(20,120,250,720),
      'CORE-01-VIS-CENTER-HEADER':(290,120,650,100),
      'CORE-01-VIS-MESSAGES':(290,240,650,220),
      'CORE-01-VIS-RUNTIME':(290,480,650,80),
      'CORE-01-VIS-COMPOSER':(290,580,650,100),
      'CORE-01-VIS-DECISION':(290,700,650,120),
      'CORE-01-VIS-RIGHT-CORE':(960,120,300,200),
      'CORE-01-VIS-RIGHT-TOPIC':(960,340,300,200),
      'CORE-01-VIS-RIGHT-VERSION':(960,560,300,260),
    }
    for row in visuals:
...
def execute():
    ...
    visual_unresolved=[
      ...
      and any(tok in str(x.get('authority_ref') or '') for tok in ('VISUAL','SHELL','HOME_SHELL','DESIGN_SYSTEM'))
      and x.get('resolved') is not True
    ]
    ...
    for row in workbench.get('atomic_workbenches') or []:
        ...
        atomic_bindings.append({
          'workbench_uid':row.get('workbench_uid'),
          'workbench_type':row.get('workbench_type'),
          'section_order':section_order,
          'visual_order':[sec_to_visual.get(str(x)) for x in section_order],
          'same_surface':row.get('same_surface'),
          'shared_context':row.get('shared_context') or [],
          'interruption_boundary':row.get('interruption_boundary'),
          'split_into_independent_surfaces':row.get('split_into_independent_surfaces'),
          'relation_to_conversation':row.get('relation_to_conversation'),
    ...
    inheritance_rows.append({
      'authority_ref':str((visual_auth.get('authority') or {}).get('id') or 'CORE01_CURRENT_CANONICAL_VISUAL_OWNER'),
      'source_path':rel(c['visual_auth_path']),
      'version':str((visual_auth.get('authority') or {}).get('version') or current_visual.get('version') or ''),
      'content_sha256':sha(c['visual_auth_path']),
      'resolution':'RESOLVED_PAGE_LOCAL_CURRENT_CANONICAL_VISUAL',
      'inherited_rule_or_component':'PAGE_LEVEL_CURRENT_CANONICAL_VISUAL_IDENTITY',
      'lock_status':'LOCKED',
      'allowed_page_local_variation':'ONLY_WHAT_CURRENT_AUTHORITY_EXPLICITLY_LEAVES_OPEN',
      'required_change_set_for_deviation':True,
    })
    for row in visual_unresolved:
        inheritance_rows.append({
          'authority_ref':row.get('authority_ref'),
          'source_path':None,
          'version':str(row.get('authority_ref') or '').split('@',1)[1] if '@' in str(row.get('authority_ref') or '') else None,
          'content_sha256':None,
          'authority_evidence_ref':row.get('authority_evidence_ref'),
          'resolution':'UNRESOLVED_AUTHORITY',
          'inherited_rule_or_component':None,
          'lock_status':'BLOCKED_UNRESOLVED',
          'allowed_page_local_variation':False,
          'required_change_set_for_deviation':True,
    ...
    annotation_row={
      'visual_uid':current_visual.get('visual_uid') or f'{page}-VIS-CURRENT',
      'page_scope_uid':page,
      'scenario_uid':f'{page}-SCENARIO-AUTHORITY-BOUNDED-REVIEWABILITY',
      'state_uid':'UNRESOLVED_APPLICABLE_VISUAL_AUTHORITY',
      'workbench_uids':workbench_uids,
      'journey_uids':journey_uids,
      'parent_visual_uid':None,
      'design_version':current_visual.get('design_version'),
      'basic_design_change_set_uid':(visual_auth.get('change_trace_contract') or {}).get('change_uid'),
      'viewport':'DESKTOP_AUTHORITY_GEOMETRY',
      'language':'zh-TW',
      'theme':'UNRESOLVED_GLOBAL_VISUAL_AUTHORITY',
      'business_entity_operations':operation_uids,
      'visible_sections':all_section_uids,
      'conditional_sections':[],
      'locked_regions':all_visual_uids,
      'editable_regions':[],
      'visual_anchor_uids':[],
      'visual_anchor_resolution':'BLOCKED_WITH_GLOBAL_VISUAL_AUTHORITY',
      'primary_controls':all_control_uids,
      'disabled_blocked_controls':[],
      'current_next_action':'RESOLVE_CORE01_STAGE03_VISUAL_AUTHORITY',
      'current_next_gate':'VISUAL_AUTHORITY_RESOLUTION_BEFORE_HUMAN_VISUAL_REVIEW',
      'source_authority_refs':source_refs,
      'inherited_visual_authority_refs':[x.get('authority_ref') for x in inheritance_rows],
      'verification_purpose':'Prove exact page geometry, complete control identity, atomic Workbench order, and the unresolved visual-Authority boundary without inventing global style.',
    }
    ...
    for kind in scenario_types:
        scenarios.append({
          'scenario_uid':f'{page}-VIS-SCENARIO-{kind}',
          'scenario_type':kind,
          'required':True,
          'coverage_status':'BLOCKED_UNRESOLVED_APPLICABLE_VISUAL_AUTHORITY',
          'workbench_uids':workbench_uids,
          'operation_uids':operation_uids,
          'control_uids':all_control_uids,
          'visual_anchor_uids':[],
          'visual_inheritance_ref':'VISUAL_INHERITANCE_MATRIX',
          'visual_candidate_ref':None,
          'reason':'GLOBAL_VISUAL_OR_SHELL_AUTHORITY_UNRESOLVED',
        })
    ...
    files={
      'VISUAL_DESIGN_SPEC_PACKAGE.yaml':design,
      'VISUAL_GEOMETRY_CONTRACT.yaml':geometry,
      'VISUAL_PREVIEW_EVIDENCE.yaml':preview,
      'VISUAL_CHANGESET.yaml':changes,
      'VISUAL_INTERACTION_TOPOLOGY_BINDING.yaml':topo,
      'FUNCTIONAL_WORKBENCH_LAYOUT_CONTRACT.yaml':wb,
      'VISUAL_REFERENCE_ANNOTATION.yaml':annotation,
      'VISUAL_INHERITANCE_MATRIX.yaml':inheritance,
      'VISUAL_SCENARIO_EVIDENCE_SET.yaml':scenario_set,
      'VISUAL_REVIEW_EVIDENCE.yaml':review,
    }
    ...
    for idx,row in enumerate(visual_unresolved,1):
        problems.append({
          'problem_uid':f'STAGE03-{page}-VISUAL-AUTHORITY-GAP-{idx:02d}',
          'page_uid':page,
          'class':'EXTERNAL_AUTHORITY_GAP',
          'category':'UNRESOLVED_APPLICABLE_VISUAL_AUTHORITY',
          'owner':'EXTERNAL_AUTHORITY_OWNER',
          'authority_ref':row.get('authority_ref'),
          'authority_evidence_ref':row.get('authority_evidence_ref'),
          'status':'OPEN',
    ...
    support={
      ...
      'EFFECTIVE_CONTRACT_OVERLAY.yaml':{**common,'artifact_type':'EFFECTIVE_CONTRACT_OVERLAY','raw_authority_refs':source_refs,'legal_successor_output_root':rel(out),'open_gap_total':open_total,'unresolved_visual_authority_refs':[x.get('authority_ref') for x in visual_unresolved]},
      'DEPENDENCY_TOPOLOGY.yaml':{**common,'artifact_type':'DEPENDENCY_TOPOLOGY','dependencies':source_refs},
      'DENOMINATOR_SNAPSHOT.yaml':{**common,'artifact_type':'DENOMINATOR_SNAPSHOT','required_visual_output_total':len(c['stage'].get('outputs') or []),'materialized_visual_output_total':len(c['stage'].get('outputs') or []),'required_control_uid_total':len(all_control_uids),'materialized_control_uid_total':len(all_control_uids),'unresolved_applicable_visual_authority_total':open_total,'open_gap_total':open_total,'closure_blocker_total':open_total,'remaining_scope_total':1},
      'CLASSIFICATION_RULESET.yaml':{**common,'artifact_type':'CLASSIFICATION_RULESET','routes':{'UNRESOLVED_APPLICABLE_VISUAL_AUTHORITY':'EXTERNAL_AUTHORITY_OWNER'}},
      'CHANGE_IMPACT_MAP.yaml':{**common,'artifact_type':'CHANGE_IMPACT_MAP','affected_units':[page],'new_visual_pattern_introduced':False,'authority_update_performed':False,'blocked_by_unresolved_authority':bool(open_total)},
      'STAGE_EXECUTION_PREFLIGHT_RECEIPT.yaml':{**common,'artifact_type':'STAGE_EXECUTION_PREFLIGHT_RECEIPT','entry_gate':'ALL_REQUIRED_PAGES_STAGE2_CLOSED','pre_execution_gate':'GOVERNANCE_LOAD_RECEIPT_PASS','status':'PASS_WITH_CURRENT_AUTHORITY_GAPS_CLASSIFIED'},
      'CURRENT_PROBLEM_REGISTER.yaml':{**common,'artifact_type':'CURRENT_PROBLEM_REGISTER','open_problem_count':open_total,'resolved_problem_count':0,'problems':problems},
      'RESOLUTION_LEDGER.yaml':{**common,'artifact_type':'RESOLUTION_LEDGER','entries':[{'problem_uid':x['problem_uid'],'disposition':'BLOCKED_EXTERNAL_AUTHORITY_REQUIRED','fresh_recheck_performed':True} for x in problems]},
    }
    ...
    scanner_truth={
      'VISUAL_DOMAIN':len(c['stage'].get('outputs') or [])==9 and len(files)>=10,
      'GEOMETRY':geometry.get('layout')==design.get('layout') and geometry.get('visual_geometry_units')==design.get('visuals'),
      'PREVIEW_IDENTITY':all(uid in svg for uid in all_control_uids),
      'FUNCTION_TO_VISUAL_BINDING':topo.get('functional_visual_impact_rows')==(impact.get('rows') or []) and topo.get('field_bindings')==(impact.get('field_bindings') or []),
      'ATOMIC_WORKBENCH_COHESION':conv.get('section_order')==['CORE-01-SEC-03','CORE-01-SEC-04','CORE-01-SEC-06','CORE-01-SEC-07'] and decision.get('section_order')==['CORE-01-SEC-05'],
      'TOPOLOGY_EQUIVALENCE':topo.get('interaction_relations')==(topology.get('edges') or []),
      'RESPONSIVE_ORDER':geometry.get('responsive_contract',{}).get('semantic_order_preserved') is True,
    }
    ...
    evidence={
      ...
      'denominator':{'required_total':len(c['stage'].get('outputs') or []),'open_gap_total':open_total,'closure_blocker_total':open_total,'remaining_scope_total':1},
      'gaps':problems,'closure_blockers':[x['problem_uid'] for x in problems],
      ...
      'remediation':{'performed':True,'discovered_gap_total':open_total,'remediated_gap_total':0,'unresolved_gap_total':open_total,'reexecution_required':True,'reexecution_performed':True,'owner_route':'EXTERNAL_AUTHORITY_GAP','reason':'UNRESOLVED_APPLICABLE_VISUAL_AUTHORITIES_PRESERVED_AFTER_FRESH_RECHECK'},
      ...
      'resume_persistence':{'performed':True,'resume_point':f'STAGE3_{page.replace("-","")}_VISUAL_AUTHORITY_BLOCKED'},
      'next_stage_transition':{'next_stage_uid':'STAGE-04','status':'BLOCKED','reason':'UNRESOLVED_APPLICABLE_VISUAL_AUTHORITY'},
    }
...
def parse_func(src,name):
    mod=ast.parse(src)
    rows=[x for x in mod.body if isinstance(x,ast.FunctionDef) and x.name==name]
    if len(rows)!=1:
        raise RuntimeError(f'REPLACEMENT_FUNCTION_INVALID:{name}')
    return rows[0]

tree=ast.parse(TARGET.read_text(encoding='utf-8'))
repls={
 'preview_svg':parse_func(PREVIEW_SRC,'preview_svg'),
 'execute':parse_func(EXECUTE_SRC,'execute'),
 'self_test':parse_func(SELFTEST_SRC,'self_test'),
}
found={k:0 for k in repls}
new=[]
for node in tree.body:
    if isinstance(node,ast.FunctionDef) and node.name in repls:
        found[node.name]+=1
        new.append(repls[node.name])
    else:
        new.append(node)
if found!={k:1 for k in repls}:
    raise RuntimeError('STRUCTURAL_FUNCTION_SELECTOR_DRIFT:'+repr(found))
tree.body=new
ast.fix_missing_locations(tree)
source=ast.unparse(tree)+'\n'
compile(source,str(TARGET),'exec')
TARGET.write_text(source,encoding='utf-8')
subprocess.run([sys.executable,str(TARGET),'--self-test'],cwd=ROOT,check=True)
subprocess.run([sys.executable,str(ROOT/'governance/ci/stage_execution_engine.py'),'--definition-audit-all'],cwd=ROOT,check=True)
print('PASS: AST-bounded Stage-03 producer patch materialized')
