...
for rec in raw_ref.get('records') or []:
    if rec.get('page_uid') == PAGE_UID and rec.get('target_path') == '00_SOURCE_INTAKE/RAW_SOURCE/CORE-01/CORE_PAGE_VISUAL_AUTHORITY_FINAL_SCRIPT_CONTENT_CLOSED.yaml':
        ...
        break
if not capture:
    die('CORE01_RAW_CAPTURE_PROVENANCE_MISSING')
if capture.get('content_mutated') is not False or capture.get('source_git_blob_sha') != capture.get('target_git_blob_sha'):
    die('CORE01_RAW_CAPTURE_NOT_BYTE_EXACT')

authority = raw.get('authority') or {}
if authority.get('id') != 'CORE_PAGE_VISUAL_AUTHORITY_FINAL' or authority.get('current_only') is not True or authority.get('status') != 'FINAL_LOCKED':
    die('CORE01_CURRENT_AUTHORITY_IDENTITY_DRIFT')
...
for base in records:
    ...
    if not uid or not category or not target:
        die(f'CORE01_R8_INCOMPLETE_IDENTITY:{uid}')
    ...
    if category == 'STATE_TRANSITION_LEDGER_FIELD_MISSING':
        ...
        if len(transition_nodes) != 1:
            die(f'CORE01_R8_TRANSITION_NODE_DENOMINATOR:{target}:{len(transition_nodes)}')
        context['transition_nodes'] = transition_nodes
    else:
        action_nodes = unique_mappings(collect(raw, lambda x: x.get('action_uid') == target))
        if not action_nodes:
            die(f'CORE01_R8_ACTION_CONTEXT_NOT_FOUND:{target}')
