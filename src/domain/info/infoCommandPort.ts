...
export async function invokeInfoCommand(input:InfoCommandInput):Promise<InfoCommandResult>{
  ...
  try{const response=await fetch(path,{method:'POST',cache:'no-store',credentials:'include',signal:input.signal,headers:{'content-type':'application/json'},body:JSON.stringify(input.payload)}),correlation_id=response.headers.get('x-correlation-id')??'unresolved',value:unknown=await response.json().catch(()=>null);if(!response.ok){const body=typeof value==='object'&&value!==null?value as Record<string,unknown>:null;return{ok:false,error_uid:errorUid(input.action_uid),reason_code:typeof body?.reason_code==='string'?body.reason_code:'INFO_COMMAND_REQUEST_FAILED',correlation_id:typeof body?.correlation_id==='string'?body.correlation_id:correlation_id};}return{ok:true,value,correlation_id};}
