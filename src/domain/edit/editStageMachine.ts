...
function next(stage:EditStageUid):EditStageUid|null{const i=EDIT_STAGE_UIDS.indexOf(stage);return i>=0&&i<EDIT_STAGE_UIDS.length-1?EDIT_STAGE_UIDS[i+1]:null;}
...
case"EXECUTION_COMPLETE":return s.phase==="RUNNING"?{...s,phase:"EVALUATING"}:s;
...
case"CONFIRM":{if(s.phase!=="WAIT_CONFIRMATION"||s.latest_result!=="PASS"||s.hard_block)return s;const n=next(s.stage_uid);return n?{stage_uid:n,phase:"READY",latest_evaluation_ref:null,latest_result:null,hard_block:false,checkpoint_ref:e.checkpoint_ref}:{...s,phase:"CONFIRMED",checkpoint_ref:e.checkpoint_ref};}
case"FINALIZE_COMPLETE":return s.stage_uid==="EDIT-01-STAGE-04-FINALIZE"?{stage_uid:"EDIT-01-STAGE-05-QA-HANDOFF",phase:"READY",latest_evaluation_ref:null,latest_result:null,hard_block:false,checkpoint_ref:s.checkpoint_ref}:s;
case"HANDOFF_COMPLETE":return s.stage_uid==="EDIT-01-STAGE-05-QA-HANDOFF"?{...s,phase:"COMPLETED"}:s;
case"BLOCK":return{...s,phase:"BLOCKED"};}}
export function canConfirmEditStage(s:EditStageState){return s.phase==="WAIT_CONFIRMATION"&&s.latest_result==="PASS"&&!s.hard_block;}
