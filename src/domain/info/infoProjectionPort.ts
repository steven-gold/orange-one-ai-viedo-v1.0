...
export async function readInfoProjection(signal?:AbortSignal){
  ...
  try{const projection=await resolver.resolve(raw);const pageState=(projection as{page_state?:unknown}).page_state;if(pageState!==null&&!isInfoPageState(pageState))throw new Error('INFO_PROJECTION_PAGE_STATE_UNREGISTERED');if(projection.test_metadata!==undefined&&!isControlledTestMode())throw new Error('INFO_TEST_PROJECTION_FORBIDDEN_IN_PRODUCTION');return{ok:true as const,projection,correlation_id};}
