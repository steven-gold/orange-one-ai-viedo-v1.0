...
import { INFO_CONTROLS, INFO_CONTROL_COUNT, infoPageText, type InfoControlSpec } from "@/i18n/infoCatalog";
import { INFO_EXPECTED_ACTION_COUNT } from "@/domain/info/infoRuntimeContract";
import { isInfoProjectionResolverBound } from "@/domain/info/infoProjectionPort";
import { isInfoCommandPayloadBuilderBound } from "@/domain/info/infoCommandPort";
...
function InfoVisualBody(){
  ...
  const valid=useMemo(()=>INFO_CONTROL_COUNT===66&&INFO_EXPECTED_ACTION_COUNT===17&&new Set(INFO_CONTROLS.map(x=>x.id)).size===66&&new Set(INFO_CONTROLS.map(x=>x.section)).size===12&&new Set(INFO_CONTROLS.map(x=>x.component)).size===20,[]);
  const pageState=state.runtime_reason_code?'ERROR':state.projection?.page_state??'LOADING';
  const projectionStatus=state.runtime_reason_code?'BLOCKED':state.projection?'READY':'LOADING';
  ...
  return <div className={styles.page} data-page-uid="workspace:INFO-01" data-vis-step="VIS-09" data-page-state={pageState} data-projection-status={projectionStatus} data-projection-reason={state.runtime_reason_code??undefined} data-correlation-id={state.correlation_id??undefined} data-projection-adapter-ready={isInfoProjectionResolverBound()?"true":"false"} data-effectful-runtime-ready={isInfoCommandPayloadBuilderBound()?"true":"false"} data-authority-sections="12" data-authority-visuals="12" data-authority-components="20" data-authority-controls="66" data-authority-actions={INFO_EXPECTED_ACTION_COUNT} data-registry-valid={valid?"true":"false"} data-fact-view={state.fact_view} data-citation-open={state.citation_open?"true":"false"} data-audit-open={state.audit_open?"true":"false"}>
    ...
    <div className={state.citation_open?styles.citationDrawerOpen:styles.citationDrawerClosed} data-citation-drawer-open={state.citation_open?"true":"false"} aria-hidden={state.citation_open?undefined:"true"}><Panel id="INFO-01-SEC-10" className={styles.lower}><div className={styles.citationGrid}>{sec10.map(x=><C key={x.id} item={x}/>)}</div></Panel></div>
    <Panel id="INFO-01-SEC-11" className={styles.lower}><div className={styles.actionDock}>{sec11.map(x=><C key={x.id} item={x}/>)}</div></Panel>
    <Panel id="INFO-01-SEC-12" className={styles.lower}><div className={styles.statusGrid}>{sec12.map(x=><C key={x.id} item={x}/>)}</div><div className={styles.notice}>{state.runtime_reason_code??infoPageText(locale,"firewall")}</div></Panel>
  </div>;
}
